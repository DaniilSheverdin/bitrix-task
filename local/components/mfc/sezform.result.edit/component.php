...
if (CModule::IncludeModule("form"))
{
    ...
    include_once __DIR__.'/component_prolog.php';
	...
	if ($arResult["ERROR"] == '')
	{
		...
		{
            // Получение ID поля для админов
            $obFieldInfo = CFormField::GetBySID('ADMIN_EMAILS', $arParams['WEB_FORM_ID']);
            if($arFieldItem = $obFieldInfo->Fetch()) {
                 $intFieldEm = $arFieldItem['ID'];
            }
            // Получение ID поля для админов
			...
			)
			{
				...
				if (check_bitrix_sessid())
				{
                    ...
                    // Заполнение значения поля email's админов
                    $intPERMISSIONVAL = 20;
                    $arPermsList = Mfc::getFormGroupAdmins($arParams["WEB_FORM_ID"], $intPERMISSIONVAL);

                    $arUserEmailsAdmin = Mfc::getFormModeratorListForMailer($arPermsList, $arResult, true);

                    if(!empty($arUserEmailsAdmin)) {
                        $listRes = CFormAnswer::GetList($intFieldEm, $fBy, $fOrd, [], $isFilt)->Fetch();
                        if($intFieldEm && isset($arResult["arrVALUES"]['form_'.$listRes['FIELD_TYPE'].'_'.$listRes['ID']])) {
                            $fBy = $fOrd = [];
                            $isFilt = true;

                            $arResult["arrVALUES"]['form_'.$listRes['FIELD_TYPE'].'_'.$listRes['ID']] = implode(', ', $arUserEmailsAdmin);
                        }
                    }
                    // Заполнение значения поля email's админов
					...
					if (CFormResult::Update($arParams["RESULT_ID"], $arResult["arrVALUES"], $arParams["EDIT_ADDITIONAL"]))
					{
                        ...
                        CFormResult::SetEvent($arParams["RESULT_ID"]);
                        CFormResult::Mail($arParams["RESULT_ID"]);
