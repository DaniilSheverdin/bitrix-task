...
if ($arResult['ACCESS']) : ?>
    <div class="page_title">
        <span><?=GetMessage('BUTTON_MZH_TABLE')?></span>

    </div>
<div class="action_row">
	<div class="search_input">
		<form class="header-search-form<?=!empty($_REQUEST['q'])?' header-search-not-empty':''?>" method="get" name="search-form" action="<?=$APPLICATION->GetCurPage()?>">
            <?
            foreach ($_GET as $k => $v) {
                if (in_array($k, array('q','quarantine'))) {
                    continue;
                }
                ?>
			...
			<div class="flt">
				<input
					class="header-search-input" name="q" type="text" autocomplete="off"
					value="<?=$_REQUEST['q']?>"
					placeholder = "искать гражданина по ФИО, адресу и ГУЗ"
				/>
				<button type="submit" class="header-search-icon header-search-icon-title"></button>
				<span class="search-title-top-delete"></span>
			</div>
			<div class="on_quarantine"><select class="main-grid-editor main-dropdown main-grid-editor-dropdown" name="quarantine"><option value="">на карантине:</option><option <?=$_REQUEST['quarantine']=="Y"?'selected':''?> value="Y">Да</option><option <?=$_REQUEST['quarantine']=="N"?'selected':''?>  value="N">Нет</option></select></div>
		</form>
	</div>
    <div id="preloader">
    ...
    <form method="post" action="/isolation/?PAGE=ADD&TYPE=MZH" enctype="multipart/form-data">
        <h3>Добавить граждан</h3>
        ...
        <button type="submit" class="button">Загрузить список</button>
