...
if (CModule::IncludeModule("form")) {
    ...
    if ($bCache && $obFormCache->InitCache($arParams["CACHE_TIME"], $CACHE_ID, $CACHE_PATH)) {
        ...
        if (
            $arParams["IGNORE_CUSTOM_TEMPLATE"] == "N"
    ...
    if (mb_strlen($arResult["ERROR"]) <= 0) {
        ...
        if (
            ($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) &&
            ...
            if (
                ...
                ||
                $arParams['USE_EXTENDED_ERRORS'] != 'Y' && mb_strlen($arResult["FORM_ERRORS"]) <= 0
            ) {
                // check user session
        ...
        */
        ...
        $arResult = array_merge(
            $arResult,
            array(
                "isFormNote"            => mb_strlen($arResult["FORM_NOTE"]) ? "Y" : "N", // flag "is there a form note"
        ...
        */
        ...
        foreach ($arResult["arQuestions"] as $key => $arQuestion) {
            ...
            if (is_array($arResult["arAnswers"][$FIELD_SID])) {
                ...
                if (is_array($arResult["arDropDown"][$FIELD_SID])) {
                    reset($arResult["arDropDown"][$FIELD_SID]);
                }
                if (is_array($arResult["arMutiselect"][$FIELD_SID])) {
                    reset($arResult["arMutiselect"][$FIELD_SID]);
                }
                    ...
                    switch ($arAnswer["FIELD_TYPE"]) {
                        case "radio":
                            ...
                            if ($arResult["isFormErrors"] == 'Y') {
                                if (
                                    mb_strpos(strtolower($arAnswer["FIELD_PARAM"]), "selected")!==false
                                    ||
                                    mb_strpos(strtolower($arAnswer["FIELD_PARAM"]), "checked")!==false
                                ) {
        ...
        $arResult["SUBMIT_BUTTON"] = "<input ".(intval($arResult["F_RIGHT"]) < 10 ? "disabled=\"disabled\"" : "")." type=\"submit\" name=\"web_form_submit\" value=\"".(mb_strlen(trim($arResult["arForm"]["BUTTON"])) <= 0 ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"])."\" />";
