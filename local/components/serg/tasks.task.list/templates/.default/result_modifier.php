...
if (!function_exists('tasksCheckRightEditPlan')) {
	function tasksCheckRightEditPlan($row)
	{
		...
		$can = 'N';
		if (
			(($row["RESPONSIBLE_ID"] == Util\User::getId()) || ($row['ACCOMPLICES'] && in_array(Util\User::getId(), $row['ACCOMPLICES']))) &&
...
if (!function_exists('prepareUF')) {
	...
	{
		if (empty($row[$fieldName]) && $row[$fieldName] !== '0') {
			return GetMessage('TASKS_NOT_PRESENT');
		}
		...
		if (is_array($row[$fieldName])) {
			...
			return htmlspecialcharsbx($row[$fieldName]);
		}
	}
}
...
if (!function_exists('prepareComplete')) {
	function prepareComplete($row, $arParams)
	{
		...
		} elseif ($row['REAL_STATUS'] == CTasks::STATE_COMPLETED) {
...
if (!empty($arResult['LIST'])) {
	...
	foreach ($arResult['LIST'] as $row) {
		if ($groupByProject && $prevGroupId != $row['GROUP_ID']) {
			...
			$arResult['ROWS']['GROUPS'][] = $groupItem;
		}
...
function getColor()
{
    ...
    for ($row = 0; $row < 1; ++$row) {
        for ($c = 0; $c < 3; ++$c) {
            $color[$c] = rand(1 + $spread, 255 - $spread);
        }
...
foreach ($arResult['ROWS']['ITEMS'] as $item) {
    if (!in_array($item['columns']['RESPONSIBLE_NAME'], $arNamesPre)) {
        $arNamesPre[] = $item['columns']['RESPONSIBLE_NAME'];
    }
}
...
foreach ($arNamesPre as $name) {
    $arNames[$count]['label'] = 1;

    $preName = explode('tasks-grid-username-inner ">', $name);

    $arNames[$count]['name'] = $preName[1];
    $arNames[$count]['color'] = getColor();
    $countGroup = 0;
    $countTask = 0;
    foreach ($arResult['ROWS']['ITEMS'] as $item) {
        if ($item['columns']['GROUP_NAME'] == '') {
            $item['columns']['GROUP_NAME'] = 'Задачи без проектов';
        }

        if ($item['columns']['RESPONSIBLE_NAME'] == $name) {
            if (!in_array($item['columns']['GROUP_NAME'], $arNames[$count]['projects'][$countGroup -1])) {
                foreach ($arResult['FILTER']['STATUS']['items'] as $key => $status) {
                    if ($status == $item['columns']['STATUS']) {
                        $arNames[$count]['status'] = ''.$key;
                    }
                }

                $arNames[$count]['projects'][$countGroup]['label'] = $countGroup +1;

                $arNames[$count]['projects'][$countGroup]['name'] = $item['columns']['GROUP_NAME'];

                $arNames[$count]['projects'][$countGroup]['color'] = getColor();

                $arNames[$count]['projects'][$countGroup]['tasks'][0]['label'] =  $countTask + 1;
                $arNames[$count]['projects'][$countGroup]['tasks'][0]['name'] = $item['columns']['TITLE'];
                $arNames[$count]['projects'][$countGroup]['tasks'][0]['color'] = getColor();

                $countGroup++;
                $countTask++;
            } else {
                $arNames[$count]['projects'][$countGroup - 1]['tasks'][$countTask]['label'] =  $countTask + 1;
                $arNames[$count]['projects'][$countGroup - 1]['tasks'][$countTask]['name'] = $item['columns']['TITLE'];
                $arNames[$count]['projects'][$countGroup - 1]['tasks'][$countTask]['color'] = getColor();
                $countTask++;
            }
        }
    }
