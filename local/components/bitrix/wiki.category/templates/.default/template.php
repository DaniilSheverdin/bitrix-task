...
if(!empty($arResult['FATAL_MESSAGE'])):
	...
	if (!empty($arResult['CATEGORIES'])):
		...
		foreach ($arResult['CATEGORIES'] as $arSect)
		{
			...
			/*
			$sCurLetter = mb_strtoupper(mb_substr(\Bitrix\Main\Text\Emoji::decode($arSect['NAME']), 0, 1));
			...
			*/
			?>
			<a title="<?=\Bitrix\Main\Text\Emoji::decode($arSect['TITLE'])?>" class="<?=($arSect['IS_RED'] == 'Y' ? 'wiki_red' : '')?>" href="<?=$arSect['LINK']?>"><?=\Bitrix\Main\Text\Emoji::decode($arSect['NAME'])?></a>
			<br/>
			<?
			$iEl++;
			if ($iEl == $iSectCount):
	...
	if (!empty($arResult['PAGES'])):
		...
		?>
		<br/><br/>
		<div class="wiki-post-header"><?=GetMessage('WIKI_PAGES_IN_SUBCATEGORY')?> "<?=\Bitrix\Main\Text\Emoji::decode($arResult['CUR_CAT']['NAME'])?>"</div>
		<div>
		<?
		foreach ($arResult['PAGES'] as $arPage)
		{
			...
			/*
			$sCurLetter = mb_strtoupper(mb_substr(\Bitrix\Main\Text\Emoji::decode($arPage['NAME']), 0, 1));
			if ($sPrevLetter != $sCurLetter):
				$sPrevLetter = $sCurLetter;
				?>
				<div><?=$sPrevLetter?></div>
			...
			*/
			?>
			<a title="<?=\Bitrix\Main\Text\Emoji::decode($arPage['TITLE'])?>" class="<?=($arPage['IS_RED'] == 'Y' ? 'wiki_red' : '')?>" href="<?=$arPage['LINK']?>"><?=\Bitrix\Main\Text\Emoji::decode($arPage['NAME'])?></a>
			<br/>
			<?
			$iEl++;
			if ($iEl == $iPageCount):
