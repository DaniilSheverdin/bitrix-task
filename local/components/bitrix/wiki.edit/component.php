...
// $arParams['ELEMENT_NAME'] = \Bitrix\Main\Text\Emoji::decode($arParams['ELEMENT_NAME']);
$arFilter = array(
...
// $sPageName = \Bitrix\Main\Text\Emoji::decode($sPageName);
$sCatName = '';
...
if ((empty($arResult['ELEMENT']) || !$bNotPage ) && $arResult['WIKI_oper']!="delete" && $arResult['WIKI_oper']!="rename" && $arResult['WIKI_oper']!="rename_it")
{
	if ($arResult['WIKI_oper'] == 'add')
	{
		...
		$sPageDecoded = \Bitrix\Main\Text\Emoji::encode($sPageDecoded);
		$arParams['ELEMENT_NAME'] = \Bitrix\Main\Text\Emoji::encode($arParams['ELEMENT_NAME']);
		$arFields=array(
...
$arResult['ELEMENT']['NAME_LOCALIZE'] = \Bitrix\Main\Text\Emoji::decode($arResult['ELEMENT']['NAME_LOCALIZE']);
$arResult['ELEMENT']['DETAIL_TEXT'] = \Bitrix\Main\Text\Emoji::decode($arResult['ELEMENT']['DETAIL_TEXT']);
$arResult['ELEMENT']['~DETAIL_TEXT'] = \Bitrix\Main\Text\Emoji::decode($arResult['ELEMENT']['~DETAIL_TEXT']);
...
while($arElement = $rsTree->GetNext())
{
	...
	$arElement['NAME'] = $_sSeparator.CWikiUtils::htmlspecialcharsback($arElement['NAME'], false);
	$arElement['NAME'] = \Bitrix\Main\Text\Emoji::decode($arElement['NAME']);
	$arTree[$arElement['NAME']] = $arElement['NAME'];
}
		...
		if (check_bitrix_sessid())
		{
			...
			switch ($arResult['WIKI_oper'])
			{
				...
				default: //add
					...
					$arResult['ELEMENT']['NAME'] = \Bitrix\Main\Text\Emoji::encode($arResult['ELEMENT']['NAME']);
				...
				case 'delete':
					...
					/*rename element, and renew all links directed on this page and/or category*/
						$newName = \Bitrix\Main\Text\Emoji::encode($_POST["NEW_NAME"]);
