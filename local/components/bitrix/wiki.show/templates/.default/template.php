		...
		case 'category' :
			...
			$APPLICATION->IncludeComponent(
				...
				Array(
					...
					'ELEMENT_NAME' => \Bitrix\Main\Text\Emoji::encode($arResult['ELEMENT']['NAME']),
					'PAGES_COUNT' => '100',
					'COLUMNS_COUNT' => '3'
				),
				$component
	...
	if (!empty($arResult['ELEMENT']['SECTIONS'])):
		?><div id="wiki_category">
			<div class="wiki-category-item">
				...
				{
					?><a title="<?=\Bitrix\Main\Text\Emoji::decode(htmlspecialcharsbx($arSect['TITLE'], ENT_QUOTES))?>" class="<?=($arSect['IS_RED'] == 'Y' ? 'wiki_red' : '')?>" href="<?=htmlspecialcharsbx($arSect['LINK'], ENT_QUOTES)?>"><?=\Bitrix\Main\Text\Emoji::decode(htmlspecialcharsbx($arSect['NAME'], ENT_QUOTES))?></a><?
