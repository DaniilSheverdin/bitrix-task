		...
		if(static::checkTasksModule())
		{
			...
			if($this->errors->checkNoFatals())
			{
	                ...
	                ($todo = static::checkExecuteDispatcher($request, $this->errors, $this->auxParams)) !== false)
                {
                    ...
                    /*
                     * 908 - проект "Закупки"
                     * UF_AUTO_AUCTION (Дата проведения аукциона) - обязательное поле для этой группы
                     * */
                    $arArguments = $todo[0]['ARGUMENTS']['data'];

                    if (
                        in_array(908, $arArguments['SE_PROJECT']) &&
                        isset($arArguments['UF_AUTO_AUCTION']) &&
                        empty($arArguments['UF_AUTO_AUCTION'])
                    ) {
                        $this->errors->add('FIELD_REQUIRED', 'Не введено значение обязательного поля "Дата проведения аукциона"', 'ERROR');
                    } else {
