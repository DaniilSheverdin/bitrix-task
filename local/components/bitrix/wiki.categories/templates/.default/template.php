...
if(!empty($arResult['FATAL_MESSAGE'])):
	...
	if (!empty($arResult['CATEGORIES']) || isset($arResult['QUERY'])):
		...
		{
			?>
			<a href="<?=$arCat['LINK']?>" title="<?=\Bitrix\Main\Text\Emoji::decode($arCat['TITLE'])?>" class="<?=($arCat['IS_RED'] == 'Y' ? 'wiki_red' : '')?>"><?=\Bitrix\Main\Text\Emoji::decode($arCat['NAME'])?></a> (<?=$arCat['CNT']?>)<br/>
			<?
		}
		?>
		<?=$arResult['DB_LIST']->NavPrint(GetMessage('NAV_TITLE'));?>
