...
if ($this->StartResultCache(false, $arParams['IBLOCK_ID'].'|'.$arResult['CAN_EDIT'].'|'.$arResult['UNDO_ID'].'|'.$cache_id))
{
	...
	if ($mode != 'subtree' || $arCurrentSection)
	{
		...
		if (isset($arParams['DEPARTMENT'])) {
		    ...
		    $arrSecIds = CIntranetUtils::GetDeparmentsTree($arParams['DEPARTMENT'], true);
		    $arrSecIds[] = $arParams['DEPARTMENT'];
		...
		while ($arRes = $dbRes->Fetch())
		{
			if (!empty($arrSecIds) && !in_array($arRes['ID'], $arrSecIds)) {
				continue;
			}
			if (isset($arParams['REMOVE_DEPTH_LEVEL'])) {
				$arRes['DEPTH_LEVEL'] = $arRes['DEPTH_LEVEL'] - $arParams['REMOVE_DEPTH_LEVEL'];
			}
