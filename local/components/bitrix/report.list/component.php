...
/**
 * Все текущие отчеты в разделе "задачи и проекты" скрыть для МФЦ 
 * @see https://corp.tularegion.local/company/personal/user/2440/tasks/
 */
if (SITE_ID != 'gi') {
...
if (empty($arResult['list']) && SITE_ID != 'gi')
{
	$dReports = call_user_func(array($helperClassName, 'getDefaultReports'));
	foreach ($dReports as $moduleVer => $vReports)
	{
