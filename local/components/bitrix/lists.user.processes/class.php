...
class ListsSelectElementComponent extends CBitrixComponent
{
	...
	public function executeComponent()
	{
		...
		$selectFields = array('ID', 'IBLOCK_TYPE_ID', 'IBLOCK_ID', 'NAME', 'DATE_CREATE', 'TIMESTAMP_X',);
		...
		while($element = $elementObject->fetch())
		{
			...
			if(!empty($documentStates))
			{
				foreach($documentStates as $documentState)
				{
					...
					$this->arResult['DATA'][$documentState['ID']]['DATE_CREATE'] = $element['DATE_CREATE'];
					$this->arResult['DATA'][$documentState['ID']]['TIMESTAMP_X'] = $element['TIMESTAMP_X'];
