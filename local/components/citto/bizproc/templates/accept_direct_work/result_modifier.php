...
$APPLICATION->SetTitle("Согласование отчёта по удалённо работающим");
...
$obLogger = new Logger('sc_accept_direct_work');
...
if ($arRequest['sc_accept_direct_work'] && $arRequest['sc_accept_direct_work'] == 'add') {
    try {
        Loader::includeModule("workflow");
        ...
        if (!empty($_FILES['DIRECT_WORK_FILE']['name']) && !in_array($_FILES['DIRECT_WORK_FILE']['type'], $arMimeTypes)) {
            throw new Exception('Файл заявления может быть только в форматах pdf');
        }
        ...
        $arResult['code'] = "OK";
        $arResult['message'] = "<p>Ваша отправлен!</p>";
    } catch (Exception $exc) {
