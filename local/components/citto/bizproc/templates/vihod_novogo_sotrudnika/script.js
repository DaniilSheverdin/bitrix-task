$(function () {
	$('.js-vihod-novogo-sotrudnika').on('submit', function (e) {
		e.preventDefault();
		e.stopPropagation();
	...
	$("select.js-work-otdel").on('change', function() {
		$.ajax({
			url: $(this).closest('form').attr('action'),
			data: { bxajaxid: $('input[name="bxajaxid"]').val(), OTDEL_ID: $(this).val() },
			processData: true,
			dataType: 'json',
			type: 'POST',
			success: function (resp) {
				let _sel = $("select.js-rukovoditel-otdela");
