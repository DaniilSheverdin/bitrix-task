$(function () {
    $('.js-vipiska_iz_etk').on('submit', function (e) {
        e.preventDefault();
        e.stopPropagation();
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.code == "ReadySign") {
                        var obFrom = $("#workarea-content").find('form#js-vipiska_iz_etk');
                        obFrom.prepend('<input type="hidden" name="documentid" value="' + resp.documentid + '" />');
                        obFrom.find('input[name="vipiska_iz_etk"]').val('signed');
    ...
    $(window).on("message onmessage", function (e) {
        ...
        if (data == 'filesigner_signed') {
            ...
            $('.js-vipiska_iz_etk').trigger('submit');
