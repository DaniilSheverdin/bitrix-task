$(function () {
    let baseID = 'js-citto-uvolnenie';
    $('.' + baseID).on('submit', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $('select[name="PROSHU_RASTORGNUT_TRUDOVOY_DOGOVOR"]').on('change', () => {
        const xmlID = $(this).find('option[data-xml-id]:selected').attr('data-xml-id');

        const docScan = $("#doc-scan");
        (xmlID == '4486a1e759e095ca7713eb41de1f3934') ? docScan.slideUp(400) : docScan.slideDown(400);
    });
});