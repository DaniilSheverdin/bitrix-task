$(function () {
    ...
    $(`.js-${IBLOCK_CODE}`).on('submit', function (e) {
        e.preventDefault();
        e.stopPropagation();
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.code == "ReadySign") {
                        var obFrom = $("#workarea-content").find(`form#js-${IBLOCK_CODE}`);
                        obFrom.prepend('<input type="hidden" name="documentid" value="' + resp.documentid + '" />');
                        obFrom.find(`input[name="${IBLOCK_CODE}"]`).val('signed');
                        ...
                        $('<iframe>', {
                            src: '/podpis-fayla/?FILES[]=' + resp.file_id + '&CHECK_SIGN=N&sessid=' + resp.sessid+'&POS=%23SIGN_USER%23',
                            id: 'popup-iframe',
                            frameborder: 0,
                            scrolling: 'no',
                            width: '100%',
