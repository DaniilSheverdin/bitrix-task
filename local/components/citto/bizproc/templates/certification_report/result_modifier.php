...
$APPLICATION->SetTitle("Отзыв при аттестации");
...
if ($arRequest['certification_report'] && $arRequest['certification_report'] == 'add') {
    try {
        Loader::includeModule("workflow");
        ...
        $arProps = [];
        $_FILES['ADD_REPORT'] = $_FILES['DOCUMENTS'];
        ...
        foreach ($arResult['FIELDS'] as $arField) {
            ...
            if ($arField['TYPE'] == 'DATE' && $sValue) {
                ...
                $arFilesInfo = [];
                foreach ($_FILES[$sCode]['name'] as $iKey => $sValue) {
                    $arFilesInfo[] = [
                        "name" => $_FILES[$sCode]['name'][$iKey],
                        "size" => $_FILES[$sCode]['size'][$iKey],
                        "tmp_name" => $_FILES[$sCode]['tmp_name'][$iKey],
                        "type" => $_FILES[$sCode]['type'][$iKey],
                    ];
                }
                $arProps[$sCode] = $arFilesInfo;
            } else {
        ...
        $arYears = [];
        foreach ($arRequest['YEARS'] as $iKey => $iYear) {
            if ($_FILES['DOCUMENTS']['name'][$iKey]) {
                array_push($arYears, $iYear);
            }
        }
        $arYears = array_unique($arYears);
        $arProps['JSON'] = json_encode([
            'YEARS' => $arYears
        ]);
...
if ($arRequest['action'] == 'birthday') {
    if ($iUserID = $arRequest['userid']) {
        $sBirthDay = $userFields($iUserID)['UF_PERSONAL_BIRTHDAY'];

        if ($sBirthDay) {
            $sBirthDay = date('Y-m-d', strtotime($sBirthDay));
        }

        $arResult['code'] = "OK";
        $arResult['ajaxid'] = $arRequest['bxajaxid'];
        $arResult['message'] = $sBirthDay;
    }
} else {
