$(function () {
	$('.js-holiday-compensation').on('submit', function (e) {
		e.preventDefault();
		e.stopPropagation();
			...
			$.ajax({
				...
				success: function (resp) {
					...
					if(resp.code == "ReadySign") {
						var obFrom = $("#workarea-content").find('form#js-holiday-compensation');
						obFrom.prepend('<input type="hidden" name="documentid" value="' + resp.documentid + '" />');
						obFrom.find('input[name="holiday-compensation"]').val('signed');
	...
	$(window).on("message onmessage", function(e) {
		...
		if(data == 'filesigner_signed') {
			...
			$('.js-holiday-compensation').trigger('submit');
