$(function () {
    ...
    $('body').on('change', 'select[name="VID_DEYATELNOSTI"]', function () {
        let sValue = $(this).find('option:selected').text();
        let obOtherField = $('#js-other-vid');
		if (sValue == 'иная') {
			obOtherField.css({'display' : 'flex'});
			obOtherField.find('input').prop('required', true);
		}
		else {
			obOtherField.hide();
			obOtherField.find('input').prop('required', false);
		}
    });

	$('body').on('change', 'select[name="OSNOVANIE_VOZNIKNOVENIYA_PRAVOVYKH_OTNOSHENIY"]', function () {
		let sValue = $(this).find('option:selected').text();
		let obOtherField = $('#js-other-osnovanie');
		if (sValue == 'иное') {
			obOtherField.css({'display' : 'flex'});
			obOtherField.find('input').prop('required', true);
		}
    ...
    $('.js-uved_inaya_rabota').on('submit', function (e) {
        e.preventDefault();
        e.stopPropagation();
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.code == "ReadySign") {
                        var obFrom = $("#workarea-content").find('form#js-uved_inaya_rabota');
