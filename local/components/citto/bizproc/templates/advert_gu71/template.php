...
?>
<form class="needs-validation js-advert_gu71" novalidate="" style="" id="advert_gu71" action="<?=htmlspecialchars($_SERVER['REQUEST_URI'])?>" method="POST" autocomplete="Off">
    <input type="hidden" name="advert_gu71" value="add">
    <?=bitrix_sessid_post()?>
                    ...
                    <? elseif ($arField['TYPE'] == 'DATE'): ?>
                        <input
                            ...
                            <? endif; ?>
                                ...
                                value="" id="bp_<?= $arField['CODE'] ?>"
                        />
                        <? if ($arField['CODE'] == 'DATE_VACCINATION'): ?>
                            <span>В случае, если двухкомпонентная вакцина, то дата вакцинации вторым компонентом!</span>
                        <? endif; ?>
                    <? endif; ?>
                </div>
            </div>
