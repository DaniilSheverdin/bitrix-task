$('body').ready(function () {
    $('.js-register_heads').on('submit', function (e) {
        e.preventDefault();
        e.stopPropagation();
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.code == "ReadySign") {
                        var obFrom = $("#workarea-content").find('form#js-register_heads');
    ...
    function eventForSelect(sCodeField) {
        $(`[name="${sCodeField}"]`).hide();

        $('body').on('change', '#' + sCodeField, function () {
            let sSelect = $(this).find('option:selected').text();
    ...
    $('[name="DATE_END_CONTRACT"]').parent().parent().hide();
    eventForSelect('OIV');
    eventForSelect('SUBORDINATE');
