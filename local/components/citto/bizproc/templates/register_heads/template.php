...
?>
<form enctype="multipart/form-data" class="needs-validation js-register_heads" id="js-register_heads" action="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>"
      method="POST" autocomplete="Off">
    <input type="hidden" name="register_heads" value="add">
    <?= bitrix_sessid_post() ?>
            ...
            >
                ...
                <div class="col-sm-8 py-2">
                    ...
                    <? elseif ($arField['TYPE'] == 'STRING'): ?>
                        <? if ($arField['CODE'] == 'OIV'): ?>
                            <select id="OIV" class="selectpicker form-control"
                                    required="required"
                                    data-dropup-auto="false" data-size="5" title="Выбрать"
                                    data-live-search="true" data-live-search-placeholder="Найти"
                            >
                                <? foreach ($arResult['DEPARTMENTS'] as $sDepartment): ?>
                            ...
                            />
                        <? elseif ($arField['CODE'] == 'SUBORDINATE'): ?>
                            <select id="SUBORDINATE" class="selectpicker form-control"
                                    required="required"
                                    data-dropup-auto="false" data-size="5" title="Выбрать"
                                    data-live-search="true" data-live-search-placeholder="Найти"
                            >
                                <? foreach ($arResult['SUBORDINATE'] as $sDepartment): ?>
                                    <option value="<?= $sDepartment ?>"><?= $sDepartment ?></option>
                                <? endforeach; ?>
                                <option value="other">Другое</option>
                            </select>
