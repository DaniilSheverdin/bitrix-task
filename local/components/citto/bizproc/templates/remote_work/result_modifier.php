...
$APPLICATION->SetTitle("Удаленная работа");
...
if ($arRequest['action'] == 'get_users') {
    $iUserID = (int) $arRequest['SOTRUDNIK'];
...
} elseif ($arRequest['uved_remote_work'] && $arRequest['uved_remote_work'] == 'add') {
    try {
        ...
        $sDateNachala = $arRequest['DATA_NACHALA'];
        $sDateOkonchaniya = $arRequest['DATA_OKONCHANIYA'];

        $iUserID = $arRequest['SOTRUDNIK'];
        $iHeadUserID = $arRequest['HEAD_OIV'];
        $iCategory = $arRequest['CATEGORY'];
        ...
        if (empty($iCategory)) {
            throw new Exception('Укажите категорию');
        }
        if (empty($sDateNachala)) {
            throw new Exception('Укажите дату начала удаленной работы');
        }
        if (empty($sDateOkonchaniya)) {
            throw new Exception('Укажите дату окончания удаленной работы');
        }
        if (strtotime($sDateNachala) > strtotime($sDateOkonchaniya)) {
            throw new Exception('Укажите корректные даты удаленной работы');
        }
        ...
        $arLoadProductArray = [
            ...
            'PROPERTY_VALUES'   => [
                ...
                'CATEGORY'          => $iCategory,
                'DATA_NACHALA'      => date('d.m.Y', strtotime($sDateNachala)),
                'DATA_OKONCHANIYA'  => date('d.m.Y', strtotime($sDateOkonchaniya)),
            ],
        ...
        $arResult['code'] = "OK";
        $arResult['message'] = "Бизнес-процесс запущен!";
    ...
    $oblistUsersAll = (new CUser())->GetList(
        $by = "LAST_NAME", 
        $order = "ASC",
        ["ACTIVE" => 'Y', "!LAST_NAME" => null, "!NAME" => null],
        ['SELECT' => ['UF_WORK_POSITION']]
    );
    ...
    $objCategory = CIBlockPropertyEnum::GetList(
        [
            "SORT" => "ASC",
            "VALUE" => "ASC"
        ],
    ...
    while ($arVal = $objCategory->Fetch()) {
        $arResult['CATEGORY'][$arVal['ID']] = $arVal;
    }
}
