...
class CittoKPIComponent extends CBitrixComponent implements Controllerable
{
    ...
    public function configureActions(): array
    {
        return [
          ...
          'addProjectInSetKP'           => ['prefilters' => []],
          'addTopManagerInSetKP'        => ['prefilters' => []],
          'deleteInSetKP'               => ['prefilters' => []],
          'saveSetKPSections'           => ['prefilters' => []],
          'saveDateLastChanges'         => ['prefilters' => []],

        ];
    }

    /** @var string */
    public const GRID_ID_RULES_CHANGE           = 'kpi_rules_change';

    /** @var string */
    public const GRID_ID_STAFF_TO_WP            = 'kpi_staff_to_wp';

    /** @var integer */
    public const SECTION_KPI_EXTRA              = SECTION_ID_KPI_EXTRA;

    /** @var integer */
    public const SECTION_ID_ALL_STRUCTURE       = SECTION_ID_CITTO_STRUCTURE;

    /** @var integer */
    public const SECTION_ID_NOTIFIES            = SECTION_ID_KPI_NOTIFIES;

    /** @var integer */
    public const SECTION_ID_PROJECTS            = SECTION_ID_KPI_KP_PROJECTS;

    /** @var integer */
    public const SECTION_ID_TOP_MANAGERS        = SECTION_ID_KPI_KP_TOP_MANAGERS;

    /** @var integer */
    public const SECTION_ID_BASE_TASKS          = SECTION_ID_KPI_KP_BASE_TASKS;

    /** @var integer */
    public const IBLOCK_ID_KPI_STRUCT           = IBLOCK_ID_KPI_STRUCTURE;

    /** @var integer */
    public const IBLOCK_ID_KPI                  = IBLOCK_ID_KPI;

    /** @var integer */
    public const IBLOCK_ID_KPI_USERS            = IBLOCK_ID_KPI_USERS;
    ...
    /** @var string */
    private $strHeadOfDepartmentID      = '41';

    /** @var array */
    private $arCitSectionsDepth         = [];

    /** @var array */
    private $arUsersAccessExtDep        = ['1460', '41', '3707', '566'];

    /** @var array */
    private $arUsersAccessExtGov        = ['1460', '3707', '566'];
    ...
    /** @var string */
    private $strWorkGroupSprintID       = '822';

    /** @var array */
    private $arTaskStatus               =
        [
    ...
    /** @var string */
    private $strIdDueAsed               = '0.YQOB.YQOD.';

    /** @var string */
    private $strAsedURI                 = 'http://172.21.254.53/';

    /** @var string */
    private $strWSDLCriticalURI         = 'ResolutionControlService/ResolutionControlService.dll/wsdl/IResolutionControlService';
    ...
    public function init(string $strPage)
    {
        ...
        switch ($strPage) {
            ...
            case 'insert_data_dep':
                ...
                $this->arResult['PERIOD'] = $this->getNowPeriodReport();
                $this->fillKPISprintDepartmentUsers($this->arResult['DEPARTMENT_DATA']);
//                $this->activateCriticalKPI();
            ...
            case 'set_kp':
                $this->arResult['ALL_PROJECTS'] = $this->getAllProjects();
                $this->arResult['ALL_PRIORITY_PROJECTS'] = $this->getPriorityProjects();

                $this->arResult['ALL_USERS'] = $this->prepareAllUsersToSelect(false, false);
                $this->arResult['ALL_TOP_MANAGERS'] = $this->getTopManagers();

                $this->arResult['KP_SECTIONS_WEIGHT'] = $this->getKPSectionsWeight();
            ...
            case 'update_data':
                switch ($_REQUEST['action']) {
                    ...
                    case 'update_user_work_position':
                        ...
                        break;
                    case 'update_user_work_position_other':
                        $this->updateWPUsersInOtherDepartments();
                        break;
                    ...
                    case 'ased':
                        ...
                        break;
                    case 'sprint':
                        $this->fillKPISprintDepartmentUsers();
                        break;
                }
    ...
    public function fillKPISprintDepartmentUsers($arDataDepartment = [])
    {


        if (count($arDataDepartment) == 0) {
            $this->getCurrentUserDepartment();
            $arDataDepartment = $this->getDataDepartment($this->intCurrentDepartmentByUser);
        }

        $strFoundFeID = '';
        $strFoundKPIID = '';

        foreach ($arDataDepartment['RESULT'] as $feID => $arDataFe) {
            foreach ($arDataFe as $userKPIID => $arUserData) {
                foreach ($arUserData['ATT_VALUE_KPI'] as $arKPI) {
                    if ($arKPI['DATA_SOURCE'] == 'Спринты') {
                        $strFoundFeID = $feID;
                        $strFoundKPIID = $arKPI['ID'];
                        break 3;
                    }
                }
            }
        ...
        foreach($arDataDepartment['RESULT'][$strFoundFeID] as $userKPIID => $arUserData) {
            $this->calculateSprintKPI($arUserData['CODE'], $userKPIID, $strFoundKPIID, $this->strWorkGroupSprintID, true);
        }
    ...
    /**
     * Расчитывает фактиеский показатель для пользователя в рабочей группе.
     *
     * @param $strUserID
     * @param $userIDKPI
     * @param $kpiID
     * @param $strWorkGroupSprintID
     * @param false $bWriteResult
     * @return float
     * @throws TasksException
     */
    public function calculateSprintKPI($strUserID, $userIDKPI, $kpiID, $strWorkGroupSprintID, bool $bWriteResult = false): float
    {
        global $DB;
        $arRes = [];
        $floatResult = 100;
        $intCountTaskClosed = 0;
        $intCountTaskClosedExpired = 0;
        $intCountTaskOpenExpired = 0;

        $arDataTasks = $this->getTasksFromWorkGroup($strUserID, $strWorkGroupSprintID);

        if (count($arDataTasks) == 0) {

            if ($bWriteResult) {
                $this->writeKPValueTasks($userIDKPI, $kpiID, $floatResult);
            }
            return $floatResult;
        }

        foreach ($arDataTasks as $arTask) {
            if ($arTask['STATUS'] == -1) {
                $intCountTaskOpenExpired++;
            } else {
                $intCompareDates = $DB->CompareDates($arTask['DEADLINE'], $arTask['CLOSED_DATE']);
                if ($intCompareDates < 0) {
                    $intCountTaskClosedExpired++;
                } else {
                    $intCountTaskClosed++;
                }
        ...
        $arRes['COMPUTED_SUM'] = round(
            $intCountTaskClosed / (($intCountTaskClosedExpired + $intCountTaskClosed) + $intCountTaskOpenExpired) * 100,
            2, PHP_ROUND_HALF_DOWN);

        $arRes['COUNTERS']['CLOSED'] = $intCountTaskClosed;
        $arRes['COUNTERS']['CLOSED_ALL'] = $intCountTaskClosedExpired + $intCountTaskClosed;
        $arRes['COUNTERS']['OPEN_EXPIRED'] = $intCountTaskOpenExpired;

        if ($arRes['COMPUTED_SUM'] <= 90) {
            $floatResult = $arRes['COMPUTED_SUM'];
        }
    ...
    /**
     * Выбирает все задачи для пользователя определенной группы
     *
     * Статус -1 (открыта, просрочена), 5 (закрыта)
     *
     *
     * @param $strUserID
     * @param $strWorkGroupSprintID
     ...
     * @throws TasksException
     */
    public function getTasksFromWorkGroup($strUserID, $strWorkGroupSprintID): array
    {
        $arRes = [];

        $obStartDatePeriodNow = new \DateTimeImmutable('first day of this month midnight');
        $strStartDatePeriodNow = $obStartDatePeriodNow->format('d.m.Y H:i:s');

        $obEndDatePeriodNow = new \DateTimeImmutable('last day of this month 23:59:59');
        $strEndDatePeriodNow = $obEndDatePeriodNow->format('d.m.Y H:i:s');


        $arOrder = ['TITLE' => 'ASC'];
        $arFilter = [
            'RESPONSIBLE_ID' => $strUserID,
            'GROUP_ID'       => $strWorkGroupSprintID,
            '>=DEADLINE'     => $strStartDatePeriodNow,
            '<=DEADLINE'     => $strEndDatePeriodNow,
            'STATUS'         => [-1, 5]
        ];
        $arSelect = ['CLOSED_DATE', 'RESPONSIBLE_ID', 'DEADLINE', 'CREATED_DATE', 'GROUP_ID', 'SITE_ID', 'TITLE', 'STATUS', 'ID'];
    ...
    /**
     * Удаляет из ИБ IBLOCK_ID_KPI_USERS уволенных сотрудников.
     *
     * @throws Exception
     */
    public function removeInactiveUsers()
    {
        ...
        $logger = new Logger('removeInactiveUsers()');
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi/remove_inactive_users.log', Logger::INFO));
        ...
        while($arFields = $rs->GetNext()) {
            $arKPIUsersIds[$arFields['CODE']] = $arFields['ID'];
        }
        ...
        $filterUsersRemove = [
            'ACTIVE'    => 'N',
            'ID'        => implode('|', array_keys($arKPIUsersIds))
        ];
        $arParamsUsersRemove = [
            'FIELDS' => ['ID', 'LAST_NAME'],
        ];

        $rsUsersRemove = CUser::GetList($by, $order, $filterUsersRemove, $arParamsUsersRemove);

        while ($arUserRemove = $rsUsersRemove->Fetch()) {

            $intRemoveUserKPIID = intval($arKPIUsersIds[$arUserRemove['ID']]);
            $arLoggerData['KPI_ID'] = $intRemoveUserKPIID;
            $arLoggerData['ID'] = $arUserRemove['ID'];
            $arLoggerData['LAST_NAME'] = $arUserRemove['LAST_NAME'];


            if(CIBlock::GetPermission(self::IBLOCK_ID_KPI_USERS) >= 'W')
            {
                $DB->StartTransaction();
                if(!CIBlockElement::Delete($intRemoveUserKPIID))
                {
                    $arLoggerData['ERROR'] .= 'Ошибка удаления элемента';
                    $DB->Rollback();
                }
                else
                    $DB->Commit();
        ...
        if ($arLoggerData) {
            $logger->info('Удален уволенный сотрудник: ', $arLoggerData);
        }
    }


    /**
     * Активация критического KPI на данных из АСЭД.
     */
    public function activateCriticalKPI()
    {
        $arDataAsed = $this->getWSDLAsedData();
        ...
        foreach ($arDataAsed as $user) {

            $filterUsers['EMAIL'] = $user['EMAIL'];

            $rsUsers = CUser::GetList($by, $order, $filterUsers, $arParamsUsers);

            while ($arUser = $rsUsers->Fetch()) {

                $arFilter['CODE'] = $arUser['ID'];

                $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
                while($arFields = $rs->GetNext()) {
                    CIBlockElement::SetPropertyValuesEx(
                        $arFields['ID'], self::IBLOCK_ID_KPI_USERS, ['ATT_KPI_CRITICAL' => $user['CRITICAL_KPI']]
                    );
                    CIBlockElement::SetPropertyValuesEx(
                        $arFields['ID'], self::IBLOCK_ID_KPI_USERS, ['ATT_COMMENT' => 'Критический KPI активирован по причине просрочки поручений в АСЭД']
                    );
                }
            }
        }
    ...
    /**
     * Получение данных с АСЭД по просроченным задачам.
     */
    public function getWSDLAsedData(string $strStartDate = null, string $strEndDate = null): array
    {

        $client = new SoapClient($this->strAsedURI.$this->strWSDLCriticalURI);

        $arRes = [];

        $requestData = [
            'DUE' => $this->strIdDueAsed,
        ];

        if ($strStartDate !== null && $strEndDate !== null) {
            $requestData['DATE_FROM'] = $strStartDate;
            $requestData['DATE_TO'] = $strEndDate;
        }

        $result = $client->get_overdue_replies_statistics(json_encode($requestData));
        ...
        if ($arResult['result'] == 'success') {
            ...
            $intCount = 0;
            foreach ($arResult['statistics'] as $staff) {

                $strTypeCriticalKPI = '';

                $intSum = intval($staff['INPROGRESS']) + intval($staff['DONE']);
                $intSumControl = intval($staff['INPROGRESS_CONTROL']) + intval($staff['DONE_CONTROL']);

                if ($intSum == 2) {
                    $strTypeCriticalKPI = '1';
                } elseif ($intSum >=3) {
                    $strTypeCriticalKPI = '2';
                }
                ...
                if ($strTypeCriticalKPI !== '' && $staff['E_MAIL']) {
                    $arRes[$intCount]['EMAIL'] = $staff['E_MAIL'];
                    $arRes[$intCount]['CRITICAL_KPI'] = $strTypeCriticalKPI;
    ...
    /**
     * Сохраняет значение фактического показателя KPI.
     *
     * @param int $userID
     * @param string $kpiID
     * @param float $floatFactResult
     */
    public function writeKPValueTasks(int $userID, string $kpiID, float $floatFactResult)
    {
        $arSelect = ['ID', 'IBLOCK_ID', 'CODE', 'PROPERTY_ATT_VALUE_KPI'];
        $arFilter = ['IBLOCK_ID' => self::IBLOCK_ID_KPI_USERS, 'ACTIVE' => 'Y', 'ID' => $userID];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
        while($arFields = $rs->GetNext()) {
            $arNewKPIValues = [];
            foreach ($arFields['PROPERTY_ATT_VALUE_KPI_VALUE'] as $count => $valueID) {
                ...
                if ($valueID == $kpiID) {
                    $strDescription = $floatFactResult;
                }
            ...
            CIBlockElement::SetPropertyValuesEx(
                $userID, self::IBLOCK_ID_KPI_USERS, ['ATT_VALUE_KPI' => $arNewKPIValues]
            );
        }
     ...
     * @throws TasksException
     */
    public function calculateKPTasks(int $userID, string $kpiID, int $userIDKPI): float
    {

        $arValuesKPTasks = $this->getKPTasksByUser($userID)['VALUES'];

        $floatFactResult = 0;


        foreach ($arValuesKPTasks as $values) {
            if ($values['TOTAL_COUNT']) {
                $floatFactResult += $values['NOT_EXPIRED_COUNT'] / $values['TOTAL_COUNT'] * $values['WEIGHT'];
            } else {
                $floatFactResult += 1 * $values['WEIGHT'];
            }
        }



        $floatFactResult = ceil($floatFactResult * 100);

        $this->writeKPValueTasks($userIDKPI, $kpiID, $floatFactResult);
    ...
    /**
     * Расчет значения показателей у пользователей отдела на основе задач на КП
     *
     * @param int $userID ID пользователя
     * @return array
     * @throws TasksException
     */
    public function getKPTasksByUser(int $userID): array
    {
        global $DB;
        $arRes = [];
        $arPriorityProjects = $this->getPriorityProjects();
        $arTopManagers      = $this->getTopManagers();

        $arWeightTasks = $this->getKPSectionsWeight();

        $arRes['1']['WEIGHT'] = floatval($arWeightTasks['PP']);
        $arRes['2']['WEIGHT'] = floatval($arWeightTasks['TM']);
        $arRes['3']['WEIGHT'] = floatval($arWeightTasks['BT']);

        $arTasksResult = [];


        $obDatePeriodNow = new \DateTimeImmutable('first day of this month midnight');
        $strDatePeriodNow = $obDatePeriodNow->format('d.m.Y H:i:s');

        $arOrderTasks    = ["TITLE" => "ASC"];
        $arSelectTasks   = [
            'ID', 'TITLE', 'CREATED_BY', 'CLOSED_DATE', 'DEADLINE', 'GROUP_ID', 'RESPONSIBLE_ID', 'CREATED_DATE'
        ];
        $arFilterTasks  = ['RESPONSIBLE_ID' => $userID, '>=CREATED_DATE' => $strDatePeriodNow];

        $rsTasks = CTasks::GetList($arOrderTasks, $arFilterTasks, $arSelectTasks);
        ...
        while ($arTask = $rsTasks->GetNext())
        {
            ...
            if ($arTask['DEADLINE'] != null) {
                $intCompareDates = $DB->CompareDates($arTask['DEADLINE'], $arTask['CLOSED_DATE']);

//TODO возможно придется убрать время для закрытых задач в этот день

                if ($intCompareDates == -1) {
                    $expiredTask = true;
                }
            }


            if (array_key_exists($arTask['GROUP_ID'], $arPriorityProjects)) {
                $arTasksResult['PRIORITY_PROJECTS'][$arTask['ID']]['TITLE'] = $arTask['TITLE'];
                $arTasksResult['PRIORITY_PROJECTS'][$arTask['ID']]['EXPIRED'] = $expiredTask;

                if (!$expiredTask) {
                    $arRes['1']['NOT_EXPIRED_COUNT']++;
                }
                $arRes['1']['TOTAL_COUNT']++;



            } elseif (array_key_exists($arTask['CREATED_BY'], $arTopManagers)) {
                $arTasksResult['TOP_MANAGERS'][$arTask['ID']]['TITLE'] = $arTask['TITLE'];
                $arTasksResult['TOP_MANAGERS'][$arTask['ID']]['EXPIRED'] = $expiredTask;

                if (!$expiredTask) {
                    $arRes['2']['NOT_EXPIRED_COUNT']++;
                }
                $arRes['2']['TOTAL_COUNT']++;

            } else {
                $arTasksResult['BASE_TASKS'][$arTask['ID']]['TITLE'] = $arTask['TITLE'];
                $arTasksResult['BASE_TASKS'][$arTask['ID']]['EXPIRED'] = $expiredTask;

                if (!$expiredTask) {
                    $arRes['3']['NOT_EXPIRED_COUNT']++;
                }
                $arRes['3']['TOTAL_COUNT']++;
        ...
        return ['VALUES' => $arRes, 'TASKS' => $arTasksResult];
    }
     ...
     * @return array
     */
    public function getKPSectionsWeight(): array
    {
        $arRes = [];

        $arSelect = ['UF_KPI_PROGRESS', 'UF_KPI_SECTION_WEIGHT'];
        $arFilter = [
            'IBLOCK_ID' => self::IBLOCK_ID_KPI,
            'ID' => [self::SECTION_ID_TOP_MANAGERS, self::SECTION_ID_BASE_TASKS, self::SECTION_ID_PROJECTS]
        ];
        $rs = CIBlockSection::GetList(['sort' => 'asc'], $arFilter, false, $arSelect);
        while ($arFields = $rs->GetNext()) {

            switch ($arFields['ID']) {
                case self::SECTION_ID_TOP_MANAGERS:
                    $arRes['TM'] = $arFields['UF_KPI_SECTION_WEIGHT'];
                    break;
                case self::SECTION_ID_BASE_TASKS:
                    $arRes['BT'] = $arFields['UF_KPI_SECTION_WEIGHT'];
                    break;
                case self::SECTION_ID_PROJECTS:
                    $arRes['PP'] = $arFields['UF_KPI_SECTION_WEIGHT'];
                    break;
            }
    ...
    /**
     * Все проекты повышенной приоритетности
     *
     * @return array
     */
    public function getTopManagers(): array
    {
        $arRes = [];

        $arSelect = ['ID', 'IBLOCK_ID', 'CODE', 'NAME'];
        $arFilter = ['IBLOCK_ID' => self::IBLOCK_ID_KPI, 'ACTIVE' => 'Y', 'IBLOCK_SECTION_ID' => self::SECTION_ID_TOP_MANAGERS ];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
        while($arFields = $rs->GetNext()) {
            $arRes[$arFields['CODE']] = $arFields;
        }
     ...
     * @return array
     */
    public function getPriorityProjects(): array
    {
        $arRes = [];

        $arSelect = ['ID', 'IBLOCK_ID', 'CODE', 'NAME'];
        $arFilter = ['IBLOCK_ID' => self::IBLOCK_ID_KPI, 'ACTIVE' => 'Y', 'IBLOCK_SECTION_ID' => self::SECTION_ID_PROJECTS ];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
        while($arFields = $rs->GetNext()) {
            $arRes[$arFields['CODE']] = $arFields;
        }
    ...
    public function getAllProjects(): array {
        ...
        $dbSubjects = CSocNetGroup::GetList(array(),
            array(),
            false,
            false,
            array('ID', 'NAME'));
        ...
        while ($arSubject = $dbSubjects->GetNext()) {
            $arRes[$arSubject['ID']] = $arSubject['NAME'];
        }
  ...
  /**
   * Выводит текущий период в фомате Месяц ГОД
   *
   * @return string
   */
    public function getNowPeriodReport(): string
    {
      ...
      return $this->arMonthDefault[$month] . ' ' . $year;
    ...
    public function getDepartmentsAccess(): array
    {
        ...
        $filterUsersHead = [
          ...
          '!UF_KPI_WORK_POSITION' => false,
          'UF_DEPARTMENT' => array_keys($arFilledDepartments),
        ];
        $arParamsUsersHead = [
          'FIELDS' => ['WORK_POSITION', 'LAST_NAME', 'NAME', 'SECOND_NAME', 'ID'],
          'SELECT' => ['UF_DEPARTMENT', 'UF_KPI_WORK_POSITION', 'UF_KPI_ACCESS_TO_DEPARTMENT']
        ];
        ...
        $arParamsUsersAccess = [
          'FIELDS' => ['WORK_POSITION', 'LAST_NAME', 'NAME', 'SECOND_NAME', 'ID'],
          'SELECT' => ['UF_KPI_WORK_POSITION', 'UF_KPI_ACCESS_TO_DEPARTMENT',]
        ];
        ...
        $arParamsUsersAssistants = [
          'FIELDS' => ['WORK_POSITION', 'LAST_NAME', 'NAME', 'SECOND_NAME', 'ID'],
          'SELECT' => ['UF_KPI_WORK_POSITION', 'UF_KPI_ASSISTANT_TO_DEPARTMENT',]
        ];
...
//        ]);
    ...
    public function calcIntegralKPI()
    {
      ...
      $arDataIntegral = [];
      $arDataCalculate = [];
      $floatSumPercentComplete = 0;
      $intCountUsersWithoutIntegralKPI = 0;


      foreach ($this->arResult['DEPARTMENT_DATA']['RESULT'] as $FEID => $funcUnit) {
        foreach ($funcUnit as $kpiUserID => $user) {
          foreach ($user['ATT_VALUE_KPI'] as $indexKPI => $kpi) {
            if ($kpi['DATA_SOURCE'] === $this->strIntegralKPIName) {
              $arDataIntegral[$kpiUserID]['NAME'] = $user['NAME'];
              $arDataIntegral[$kpiUserID]['ATT_VALUE_KPI'][$indexKPI]['ID'] = $kpi['ID'];
              $arDataIntegral[$kpiUserID]['ATT_VALUE_KPI'][$indexKPI]['VALUE'] = $kpi['VALUE'];
              $arDataIntegral[$kpiUserID]['ATT_VALUE_KPI'][$indexKPI]['IS_INTEGRAL'] = true;
            } else {
              $arDataCalculate[$kpiUserID]['RESULT_KPI'] = $user['ATT_RESULT_KPI'];
              $arDataCalculate[$kpiUserID]['PERCENT_COMPLETE'] = $user['ATT_PERCENT_COMPLETE'];
              $arDataCalculate[$kpiUserID]['USER_ID'] = $user['CODE'];
              $arDataCalculate[$kpiUserID]['NAME'] = $user['NAME'];
              $arDataCalculate[$kpiUserID]['KPI'][$indexKPI]['ID'] = $kpi['ID'];
              $arDataCalculate[$kpiUserID]['KPI'][$indexKPI]['VALUE'] = $kpi['VALUE'];
              $arDataCalculate[$kpiUserID]['KPI'][$indexKPI]['TARGET_VALUE'] = $kpi['TARGET_VALUE'];
            }
          }
        }
      }

      foreach ($arDataCalculate as $userID => $user) {
        if (!in_array($userID, array_keys($arDataIntegral))) {

          $rsUser = CUser::GetByID(intval($user['USER_ID']));
          $arUser = $rsUser->Fetch();

          if (in_array(mb_strtolower($arUser['WORK_POSITION']), $this->arIntegralWP)) {

            $floatSumPercentComplete += $user['PERCENT_COMPLETE'];
            $intCountUsersWithoutIntegralKPI++;

          }
        } else {

          foreach ($user['KPI'] as $indexKPI => $kpi) {
            $arDataIntegral[$userID]['ATT_VALUE_KPI'][$indexKPI]['ID'] = $kpi['ID'];
            $arDataIntegral[$userID]['ATT_VALUE_KPI'][$indexKPI]['VALUE'] = $kpi['VALUE'];
            $arDataIntegral[$userID]['ATT_VALUE_KPI'][$indexKPI]['IS_INTEGRAL'] = false;

          }
        }
      }

      $floatResultIntegralKPI = round($floatSumPercentComplete / $intCountUsersWithoutIntegralKPI, 2, PHP_ROUND_HALF_DOWN);

      foreach ($arDataIntegral as $userID => &$data) {

        $arKPIValues = [];

        ksort($data['ATT_VALUE_KPI'], SORT_NUMERIC);
        foreach ($data['ATT_VALUE_KPI'] as $index => &$kpi) {
          if ($kpi['IS_INTEGRAL']) {
            $kpi['VALUE'] = $floatResultIntegralKPI;
          }

          $arKPIValues[$index] = ['VALUE' => intval($kpi['ID']), 'DESCRIPTION' => $kpi['VALUE']];
        }
   ...
   */
    ...
    public function saveRetroData() {
        ...
        if ($this->bForceWriteRetroData || $this->checkGovernmentData() <= 0) {
            ...
            $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_save_retro_data.log', Logger::INFO));
    ...
    public function setAccessToUser()
    {
        ...
        $arParams = [
          'FIELDS' => ['WORK_POSITION', 'LAST_NAME', 'NAME', 'SECOND_NAME', 'ID'],
          'SELECT' => ['UF_DEPARTMENT', 'UF_KPI_WORK_POSITION']
        ];
    ...
    public function getGovernmentInformationSystem(string $govID) {
        ...
        foreach ($this->arCitSectionsDepth['child'] as $id => $gov) {
            if ($gov['ID'] == $govID) {
                $arRes = $gov;
            }
        }
    ...
    public function makeWorkPositions()
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_make_work_positions.log', Logger::INFO));
    ...
    /**
     * WARNING: Метод изменяет данные в пользовательском поле UF_KPI_WORK_POSITION без подтверждения.
     * Привязка осуществляется по доп полю UF_KPI_WORK_POSITION в объекте USER.
     * Присутствует дополнительная проверка на привязку к другому отделу.
     *
     * @throws Exception Без комментариев.
     * @return void
     */
    public function updateWPUsersInOtherDepartments()
    {
        ...
        $loggerDebug = new Logger('OTHER_DEPARTMENT');
        $loggerDebug->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_add_work_position.log', Logger::INFO));
        ...
        foreach ($this->arUsers as $key => $user) {
            ...
            $departmentID = 0;
            $arSelectKPIUsers = ['ID', 'IBLOCK_ID', 'CODE', 'PROPERTY_ATT_DEPARTMENT', 'PROPERTY_ATT_OTHER_DEPARTMENT',];
            $arFilterKPIUsers = ['IBLOCK_ID' => self::IBLOCK_ID_KPI_USERS, 'ACTIVE' => 'Y', 'CODE' => $user['ID']];
            $rsKPIUsers = CIBlockElement::GetList(['sort' => 'asc'], $arFilterKPIUsers, false, false, $arSelectKPIUsers);
            while($arFieldsKPIUsers = $rsKPIUsers->GetNext()) {

                $departmentID = $arFieldsKPIUsers['PROPERTY_ATT_DEPARTMENT_VALUE'];
                if ($arFieldsKPIUsers['PROPERTY_ATT_OTHER_DEPARTMENT_VALUE']) {
                    $departmentID = $arFieldsKPIUsers['PROPERTY_ATT_OTHER_DEPARTMENT_VALUE'];
                }
            }
            ...
            if ($intWorkPositionID == 0) {
                ...
                $arUpdateUser = ['UF_KPI_WORK_POSITION' => $intWorkPositionID];
     ...
     * @param bool $selfDepartment
     * @param bool $excludeSelfDepartment
     * @return array
     */
    public function prepareAllUsersToSelect(bool $selfDepartment = false, bool $excludeSelfDepartment = true): array
    {
        $this->getCitSections();
...
//                pre($arUser);
                ...
                while ($arFieldsFU = $rsFU->GetNext()) {
                    ...
                    if ($arFieldsFU['PROPERTY_ATT_OTHER_DEPARTMENT_VALUE'] == $this->intCurrentDepartmentByUser && $excludeSelfDepartment) {
                        unset($arData[$id]);
                    }
                }
            }
    ...
    public function calculateKPIValue(int $intUserIDs, bool $realValue, $log = true): float
    {
        if ($log) {
            ...
            $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_update_user_kpi.log', Logger::INFO));
        }
        ...
        $arSelectSectionExt = ["UF_FORMULA_EXT_CRITICAL", "UF_FORMULA_EXT_PROGRESS", "UF_KPI_PROGRESS"];
        ...
        while ($arWPF = $rsWPF->GetNext()) {
            $arFormulaCritical = explode(',', $arWPF['UF_FORMULA_EXT_CRITICAL']);
            $arFormulaProgress = explode(',', $arWPF['UF_FORMULA_EXT_PROGRESS']);
            $strProgressValue = $arWPF['UF_KPI_PROGRESS'];
        }
     ...
     * @throws Exception
     */
    public function getDataDepartment(int $intDepartmentID): array
    {
        $arRes = [];
...
// TODO как переписать выражение?
                ...
                $arRes[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']][$arFields['ID']]['CODE'] = $arFields['CODE'];
                $arRes[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']][$arFields['ID']]['WPID'] = intval($arUser['UF_KPI_WORK_POSITION']);
                ...
                $arRes[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']][$arFields['ID']]['ATT_PERCENT_COMPLETE'] = $arFields['PROPERTY_ATT_PERCENT_COMPLETE_VALUE'];
                        ...
                        if (intval($kpiValueID) > 0) {
                            ...
                            while ($arFieldsKPIValue = $rsKPIValue->GetNext()) {
                                ...
                                if (!in_array($arFieldsKPIValue['NAME'], $arKPILabels[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']]) && $arFieldsKPIValue['NAME']) {
                                    ...
                                    $arKPILabels[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']][$arFieldsKPIValue['ID']]['TYPE'] = $strDataSourceName;
                                }
                            }
                            ...
                            if ($strDataSourceName == 'Поручения на КП') {


//                                $this->calculateKPTasks(intval($arFields['CODE']), $kpiValueID, intval($arFields['ID']));
                            }
                ...
                $arSelectFE = [
                    ...
                    'PROPERTY_ATT_DATE_LAST_CHANGES'
                ];
                $arFilterFE = [
                ...
                while ($arFieldsFE = $rsFE->GetNext()) {
                    ...
                    $arWorkPositionDateLastChanges[$arFields['PROPERTY_ATT_WORK_POSITION_VALUE']] = date('d.m.Y H:i', strtotime($arFieldsFE['PROPERTY_ATT_DATE_LAST_CHANGES_VALUE']));
                }
...
//              'WP_LAST_CHANGES' => $arWorkPositionDateLastChanges,
...
//            ]);
    ...
    public function saveKPIExt(bool $boolForceMethod = false)
    {
        ...
        $arSelectSectionExt = ["UF_KPI_FACTS_CRITICAL"];
        $arFilterSectionExt = ["IBLOCK_ID" => self::IBLOCK_ID_KPI, "ID" => self::SECTION_KPI_EXTRA];
        $arFilterSectionExt["UF_KPI_FACTS_CRITICAL"] = false;

        $rsWPF = CIBlockSection::GetList(['sort' => 'asc'], $arFilterSectionExt, false, $arSelectSectionExt);
        if ($rsWPF->SelectedRowsCount()) {
            $this->updateSection(self::SECTION_KPI_EXTRA, ["UF_KPI_FACTS_CRITICAL" => 'empty']);
        }

        $arFilterSectionExt["!UF_KPI_FACTS_CRITICAL"] = 'empty';
        if ($boolForceMethod) {
            unset($arFilterSectionExt["UF_KPI_FACTS_CRITICAL"]);
            unset($arFilterSectionExt["!UF_KPI_FACTS_CRITICAL"]);
        }
        ...
        if ($rsWP->SelectedRowsCount() && count($arSaveKPIs) > 0) {
            $this->updateSection(self::SECTION_KPI_EXTRA, ["UF_KPI_FACTS_CRITICAL" => serialize($arSaveKPIs)]);
        }
    }
    ...
    public function getRulesExtra(int $intID = 0)
    {
        ...
        $rsSection = CIBlockSection::GetList(['sort' => 'asc'], $arFilter, false, ['UF_KPI_PROGRESS']);
        while ($arSection = $rsSection->GetNext()) {
            $arRes['PROGRESS']['VALUE'] = $arSection['UF_KPI_PROGRESS'];
        }
            ...
            if ($boolForceMethod) {
                ...
                if (count($arSaveKPIs) > 0) {
                    ...
                    while ($arFields = $rs->GetNext()) {
                        ...
                        $loggerDebug->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_debug.log', Logger::INFO));
...
// TODO Возможен косяк (не все разделы подтянет).
    ...
    public function updateGridDepartmentUsers()
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_upd_grid_users.log', Logger::INFO));
    ...
    protected function deleteElements(int $IBLOCK_ID)
    {
        ...
        foreach ($_REQUEST['ID'] as $elementID) {
            if (intval($elementID) > 0) {
                ...
                $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_rules_change.log', Logger::INFO));
...
// }
        ...
        $arFormula = [];
        $arSelectCritical = ["UF_FORMULA_EXT_CRITICAL"];
        ...
        while ($arFields = $rsCritical->GetNext()) {
            $arFormula = explode(',', $arFields['UF_FORMULA_EXT_CRITICAL']);
        }
    ...
    public function getKPIProgress(string $strValueCritical)
    {
        ...
        $arFormula = [];
        $arSelectCritical = ['UF_KPI_PROGRESS', 'UF_FORMULA_EXT_PROGRESS'];
        $arFilterCritical = ["IBLOCK_ID" => self::IBLOCK_ID_KPI, "ID" => self::SECTION_KPI_EXTRA];
        $rsCritical = CIBlockSection::GetList(['sort' => 'asc'], $arFilterCritical, false, $arSelectCritical);
        while ($arFields = $rsCritical->GetNext()) {

            $floatKPIProgressValue = $arFields['UF_KPI_PROGRESS'];

            $arFormula = explode(',', $arFields['UF_FORMULA_EXT_PROGRESS']);
        }
    ...
    public function saveUserKPI(array $arUser)
    {
        ...
        $logger->pushHandler(
            new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_update_user_kpi.log', Logger::INFO)
        );
    ...
    public function getNextLabelAction($work_position)
    {
        ...
        $arFilter = ["IBLOCK_ID" => IBLOCK_ID_KPI, "ACTIVE" => "Y", "PROPERTY_ATT_WORK_POSITION" => intval($work_position)];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
        while ($arFields = $rs->GetNext()) {
    ...
    public function addKPIAction(
        ...
        $loggerDebug = new Logger('DEBUG');
        $loggerDebug->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_add.log', Logger::INFO));
    ...
    public function returnSavedKPIAction($work_position)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_return.log', Logger::INFO));
...
// элемент изменен текущим пользователем
    ...
    public function addRuleExtraAction($NAME, $ATT_FACT, $ATT_LABEL)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_extra.log', Logger::INFO));
    ...
    public function saveRulesExtraAction($data)
    {
        ...
        foreach ($data as $item) {
            ...
            $arSave['UF_KPI_PROGRESS'] = $arItem['progress_value'];
            $arSave['UF_FORMULA_EXT_CRITICAL'] = $arItem['formula_critical'];
            $arSave['UF_FORMULA_EXT_PROGRESS'] = $arItem['formula_progress'];
    ...
    public function returnSavedKPIExtAction()
    {
        ...
        $arSelectWP = ["UF_KPI_FACTS_CRITICAL"];
        ...
        while ($arFields = $rsWP->GetNext()) {
            $arSaveKPIs = unserialize(htmlspecialcharsBack($arFields['UF_KPI_FACTS_CRITICAL']));
        }
    ...
    public function getFormulasExtAction()
    {
        ...
        $arSelect = ["UF_FORMULA_EXT_CRITICAL", "UF_FORMULA_EXT_PROGRESS"];
        $arFilter = ["IBLOCK_ID" => self::IBLOCK_ID_KPI, "ACTIVE" => "Y", "ID" => self::SECTION_KPI_EXTRA];
        $rs = CIBlockSection::GetList(['sort' => 'asc'], $arFilter, false, $arSelect);
        while ($arFields = $rs->GetNext()) {
            $arFormulas['critical'] = $arFields['UF_FORMULA_EXT_CRITICAL'];
            $arFormulas['progress'] = $arFields['UF_FORMULA_EXT_PROGRESS'];
        }
                    ...
                    foreach ($arStaff as $userID => $arUser) {
                        ...
                        foreach ($arUser['ATT_VALUE_KPI'] as $kpi) {
                            ...
                            if ($kpi['VALUE']) {
                                ...
                                $achievePercent = round($achievePercent, 2, PHP_ROUND_HALF_DOWN);
                            ...
                            $salaryPercent = $this->calculateKPIValue($userID, false);
                        ...
                        if ($percentComplete > 0) {
                            ...
                            $percentComplete = round($percentComplete, 2, PHP_ROUND_HALF_DOWN);
                        ...
                        if ($arUser['NAME'] && $percentComplete >= 40) {
                            $percentKPI =  floor($this->calculateKPIValue($userID, false));
...
// TODO Send file to email by id
    ...
    public function createWorkPositionAction($department, $new_work_position)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_create_work_positions.log', Logger::INFO));
    ...
    public function addAccessToUsersAction($data)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/kpi_add_access_to_department.log', Logger::INFO));
...
//            }
    ...
    public function addProjectInSetKPAction($project_id, $project_name) {


        if (!CIBlockElement::GetList([], ['IBLOCK_ID' => self::IBLOCK_ID_KPI, '=CODE' => $project_id])->Fetch()) {

            global $USER;
            $obPriorityProject = new CIBlockElement();



            $arLoadPriorityProject = Array(
                'MODIFIED_BY'       => $USER->GetID(),
                'IBLOCK_SECTION_ID' => self::SECTION_ID_PROJECTS,
                'IBLOCK_ID'         => self::IBLOCK_ID_KPI,
                'NAME'              => $project_name,
                'ACTIVE'            => 'Y',
                'CODE'              => $project_id,
            ...
            if($intPPID = $obPriorityProject->Add($arLoadPriorityProject)) {
                return [
                    'create_id' => $intPPID,
                ...
                return "Error: ".$obPriorityProject->LAST_ERROR;
            }
        } else {
            return [
                'create_id' => 0,
    ...
    public function addTopManagerInSetKPAction($user_id, $user_name) {


        if (!CIBlockElement::GetList([], ['IBLOCK_ID' => self::IBLOCK_ID_KPI, '=CODE' => $user_id])->Fetch()) {

            global $USER;
            $obTopManager = new CIBlockElement();



            $arLoadPriorityProject = Array(
                'MODIFIED_BY'       => $USER->GetID(),
                'IBLOCK_SECTION_ID' => self::SECTION_ID_TOP_MANAGERS,
                'IBLOCK_ID'         => self::IBLOCK_ID_KPI,
                'NAME'              => $user_name,
                'ACTIVE'            => 'Y',
                'CODE'              => $user_id,
            ...
            if($intTMID = $obTopManager->Add($arLoadPriorityProject)) {
                return [
                    'create_id' => $intTMID,
                    'user_id' => $user_id,
                    'user_name' => $user_name,
                ...
                return "Error: ".$obTopManager->LAST_ERROR;
            }
        } else {
            return [
                'create_id' => 0,
    ...
    public function deleteInSetKPAction($id) {
        ...
        if(CIBlock::GetPermission(self::IBLOCK_ID_KPI)>='W')
        {
            ...
            {
                $DB->Rollback();
                return ['delete_id' => 0,'message' => 'Недостаточно прав'];
            }
            else {
                $DB->Commit();
                return ['delete_id' => $id];
    ...
    public function saveSetKPSectionsAction($section_projects, $section_top_managers, $section_base_tasks) {

        $bt = floatval($section_base_tasks);
        $tm = floatval($section_top_managers);
        $pp = floatval($section_projects);

        if (($bt + $tm + $pp) == 1) {
            $this->updateSection(self::SECTION_ID_PROJECTS, ['UF_KPI_SECTION_WEIGHT' => $section_projects]);
            $this->updateSection(self::SECTION_ID_TOP_MANAGERS, ['UF_KPI_SECTION_WEIGHT' => $section_top_managers]);
            $this->updateSection(self::SECTION_ID_BASE_TASKS, ['UF_KPI_SECTION_WEIGHT' => $section_base_tasks]);

            return ['status' => true, 'message' => 'Данные сохранены'];

        } else {

            return ['status' => false, 'message' => 'Сумма значений должна равняться 1'];
        }

    }

    public function saveDateLastChangesAction($wpid) {

        $workPositionID = intval($wpid);
        if ($workPositionID > 0) {

            $obDateNow = new \DateTimeImmutable('now');
            $strDateNow = $obDateNow->format('d.m.Y H:i:s');

            CIBlockElement::SetPropertyValuesEx(
                $workPositionID, self::IBLOCK_ID_KPI_STRUCT, ['ATT_DATE_LAST_CHANGES' => $strDateNow]
            );

        }

        return [$workPositionID, $strDateNow];
