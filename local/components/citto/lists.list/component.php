...
foreach ($arResult["FILTER"] as $filterRow) {
    if ($filterRow['dateFilter']) {
...
foreach ($cfilterOptions as $cfilterOption) {
    if (isset($filterData[$cfilterOption])) {
        continue;
    }
    if (!isset($_REQUEST[$cfilterOption])) {
        continue;
    }
    $filterData[$cfilterOption] = $_REQUEST[$cfilterOption];
}
...
while($obElement = $rsElements->GetNextElement())
{
	...
	if(
		!$arResult["IS_SOCNET_GROUP_CLOSED"] && ($lists_perm >= CListPermissions::CAN_WRITE
