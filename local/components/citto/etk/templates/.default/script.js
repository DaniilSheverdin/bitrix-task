$(document).ready(function(){
        ...
        if (users.length > 0) {
            ...
            modal.find('form').trigger('reset').off('submit').on('submit', function(e) {
                let wait = BX.showWait('modalStep1'),
                    ...
                    request = BX.ajax.runComponentAction('citto:etk', 'step1', {
                        mode: 'ajax',
                        data: {
                            action: 'step1',
                            request: formData
    ...
    $('.js-remove-file').off('click').on('click', function(){
        let userId = $(this).attr('data-user'),
            ...
            request = BX.ajax.runComponentAction('citto:etk', 'removefile', {
                mode: 'ajax',
                data: {
                    action: 'removefile',
                    user: userId,
    ...
    $('.js-hide-user').off('click').on('click', function(){
        let $this = $(this),
            ...
            request = BX.ajax.runComponentAction('citto:etk', 'hideuser', {
                mode: 'ajax',
                data: {
                    action: 'hideuser',
                    user: userId
    ...
    $('.js-show-user').off('click').on('click', function(){
        let $this = $(this),
            userId = $this.attr('data-user'),
            request = BX.ajax.runComponentAction('citto:etk', 'showuser', {
                mode: 'ajax',
                data: {
                    action: 'showuser',
                    user: userId
