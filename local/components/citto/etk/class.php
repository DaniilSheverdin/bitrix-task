...
class Component extends CBitrixComponent
{
    ...
    public function executeComponent()
    {
        try {
            ...
            $iblockFiles = $helper->getIblockId('bizproc_etk_simple', 'bitrix_processes');

            $arHiddenUsers = $this->getHiddenUsers();

            $obCache = Cache::createInstance();
            if ($obCache->initCache(3600, "getUserList_".md5(serialize($_REQUEST)), '/etk/')) {
            ...
            $this->arResult['EXPORT'] = [
                'TITLE'     => 'Уведомления ЭТК',
                'FILENAME'  => 'Уведомления ЭТК',
                'HEADERS'   => [
                    'USER'      => [
                        'NAME'  => 'Сотрудник',
                        'WIDTH' => 50,
            ...
            while ($row = $res->GetNext()) {
                foreach ($row['PROPERTY_FILES_VALUE'] as $list) {
    ...
    {
        $file = $_SERVER['DOCUMENT_ROOT'] . '/etk/hidden_users.json';
        if (file_exists($file)) {
            try {
