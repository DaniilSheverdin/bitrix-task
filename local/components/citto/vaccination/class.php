...
class CittoVacComponent extends CBitrixComponent implements Controllerable
{
    ...
    private $arTypesVaccinations = null;
    ...
    public function init(string $strPage)
    {
        ...
        switch ($strPage) {
            case 'write':
                $this->arResult['ACCESS_TO_SECOND'] = $this->checkRulesForSecondWriteCovid();
                break;
        }
    }
    ...
    public function makeGridData()
    {
        ...
        $arItemsVaccination = [];
        foreach ($this->getTypesVaccinations() as $arItem) {
            $arItemsVaccination[$arItem['ID']] = $arItem['VALUE'];
        }
        ...
        $this->arResult['UI_FILTER'] = [
            ...
            [
                'id' => 'ATT_TYPE_VACCINATION',
                'name' => 'Тип вакцинации',
                'type' => 'list',
                'items' => $arItemsVaccination,
                'default' => true,
            ],
        ];
        ...
        foreach ($filterData as $k => $v) {
            ...
            if ($k == 'ATT_TYPE_VACCINATION' && $v) {
                $this->arResult['FILTER_DATA']['PROPERTY_ATT_TYPE_VACCINATION'] = $v;
            }

            if ($k == 'FIND' && $v) {
                $this->arResult['FILTER_DATA']['NAME'] = '%'.$v.'%';
            }
        }
        ...
        $this->arResult['columns'][] = [
            'id' => 'ATT_TYPE_VACCINATION', 'name' => 'Тип вакцинации', 'sort' => 'NAME', 'default' => true, 'editable' => false
        ];
        ...
        while ($arFieldsDataGrid = $rsDataGrid->GetNext()) {
            ...
            $this->arResult['list'][] = [
                'data' => [
                    ...
                    "ATT_TYPE_VACCINATION" => $arFieldsDataGrid['PROPERTY_ATT_TYPE_VACCINATION_VALUE'],
    ...
    public function fetchDataByUser()
    {
        ...
        $arRes = [];
        $bEditable = true;
        $arTypeVaccination = $this->getTypesVaccinations();
        ...
        while ($arFields = $rs->GetNext()) {
            $iVaccinationID = $arFields['PROPERTY_ATT_TYPE_VACCINATION_ENUM_ID'];
            foreach ($arTypeVaccination as $sType => $arType) {
                if ($arType['ID'] == $iVaccinationID) {
                    $arVaccinations[$arType['XML_ID']][] = $arFields['ID'];
                    break;
                }
            }
            ...
            $arRes[$arFields['ID']]['VACCINATION_ID'] = $iVaccinationID;
        }
        if (count($arRes)) {
            $bEditable = false;
        }

        return ['DATA' => $arRes, 'VACCINATIONS_IDS' => $arVaccinations, 'EDITABLE' => $bEditable];

    }

    public function checkRulesForSecondWriteCovid()
    {
        global $USER;
        ...
        $arFilter = [
            ...
            'PROPERTY_ATT_TYPE_VACCINATION' => $arTypeVaccination['covid-19']['ID']
        ];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
    ...
    public function checkRulesForNewWriteCovid()
    {
        global $USER;
    ...
    public function writeDataAction($data)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/vaccination_new/add.log', Logger::INFO));
        ...
        $strDate = '';
        $strTime = '';
        $arTypesVaccinations = $this->getTypesVaccinations();
        ...
        if (!$arProps['ATT_TYPE_VACCINATION']) {
            $arProps['ATT_TYPE_VACCINATION'] = $arTypesVaccinations['covid-19']['ID'];
        }
        ...
        $arFilterCheck = ['IBLOCK_ID' => self::IBLOCK_ID_VACCINATION, 'ACTIVE' => 'Y', 'PROPERTY_ATT_TYPE_VACCINATION' => $arProps['ATT_TYPE_VACCINATION']];
        ...
        if ($intNewWriteID = $objWrite->Add($arLoadWrite)) {
            ...
            $arDataEmail = [
                ...
                'THEME' => ($arProps['ATT_TYPE_VACCINATION'] == $arTypesVaccinations['covid-19']['ID']) ? 'Запись на вакцинацию от covid-19' : 'Запись на вакцинацию от гриппа'
            ];

            $arFilesIDs = [];
            $arFiles = [];

            if ($arProps['ATT_TYPE_VACCINATION'] == $arTypesVaccinations['covid-19']['ID']) {
                ...
                $arRes['files'][] = $this->generateFile($arDataFile, 'dobrovolnoe_informirovanie' . $strCode, 'vacc_information');
            }
    ...
    public function newRecordCovidAction()
    {
        return $this->checkRulesForNewWriteCovid();
    }
    ...
    public function generateFile($arData, $strFileName, $strTemplateName)
    {
        ...
        $strTemplatePath = '/local/components/citto/vaccination/templates/main/';
        $strDirSave = 'docs';
    ...
    public function deleteWriteAction($id)
    {
        ...
        $logger->pushHandler(new StreamHandler($_SERVER['DOCUMENT_ROOT'] . '/local/logs/vaccination_new/delete.log', Logger::INFO));
    ...
    public function getFreeTimesAction($data)
    {
        $obData = json_decode($data);
        $sTypeVaccination = ($obData->type_vaccination) ? $obData->type_vaccination : 'covid-19';
        ...
        $arFilter = [
            ...
            'PROPERTY_ATT_TYPE_VACCINATION' => (int) $this->getTypesVaccinations()[$sTypeVaccination]['ID']
        ];
        $rs = CIBlockElement::GetList(['sort' => 'asc'], $arFilter, false, false, $arSelect);
        while ($arFields = $rs->GetNext()) {
        ...
        foreach ($arDefaultDateTimeList as $time) {
            if ($sTypeVaccination != 'covid-19') {
                ...
                if (strtotime($time) > strtotime($sEndTime) || strtotime($time) < strtotime($sStartDate)) {
                    continue;
                }
            }
    ...
    public function getTypesVaccinations()
    {
        if (is_null($this->arTypesVaccinations)) {
            $arEnums = [];
            $obEnumList = CIBlockProperty::GetPropertyEnum("ATT_TYPE_VACCINATION", [], ["IBLOCK_ID" => self::IBLOCK_ID_VACCINATION]);
            while ($arEnum = $obEnumList->GetNext()) {
                $arEnums[$arEnum['XML_ID']] = $arEnum;
            }

            $this->arTypesVaccinations = $arEnums;
        }

        return $this->arTypesVaccinations;
    }
    ...
    public function executeComponent()
    {
        ...
        $this->arResult['TYPES_VACCINATION'] = $this->getTypesVaccinations();
