...
if (mb_strlen($arParams["MONTH_VAR_NAME"])<=0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $arParams["MONTH_VAR_NAME"])) {
    $arParams["MONTH_VAR_NAME"] = "month";
}
$arParams["YEAR_VAR_NAME"] = trim($arParams["YEAR_VAR_NAME"]);
if (mb_strlen($arParams["YEAR_VAR_NAME"])<=0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $arParams["YEAR_VAR_NAME"])) {
...
if ($arParams["TYPE"] || !in_array($arParams["DATE_FIELD"], array("DATE_ACTIVE_FROM", "DATE_ACTIVE_TO", "TIMESTAMP_X", "DATE_CREATE",'PROPERTY_DATE_ISPOLN'))) {
...
if ($arParams["TYPE"]) {   //Do not show future news
...
if ($this->StartResultCache(false, array($currentMonth, $currentYear, $todayYear, $todayMonth, $todayDay))) {
    ...
    if (mb_strlen($arParams["MONTH_URL"])<=0) {
    ...
    $arFilter = array(
            "ACTIVE" => "Y",
            ">=".$arParams["DATE_FIELD"] => date('Y-m-d H:i:s', mktime(0, 0, 0, $currentMonth, 1, $currentYear)),
            "<".$arParams["DATE_FIELD"] => date('Y-m-d H:i:s', mktime(0, 0, 0, $currentMonth+1, 1, $currentYear)),
            "IBLOCK_ID" => $arParams["IBLOCK_ID"]
        );

    $arSelectedFields = array("ACTIVE", $arParams["DATE_FIELD"], "ID", "IBLOCK_ID", "SITE_ID", "DETAIL_PAGE_URL", "NAME", "LANG_DIR", "SORT", "IBLOCK_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE",'PROPERTY_DATE_ISPOLN');
    ...
    while ($arItem = $dbItems->GetNext()) {
        $arDays[ConvertDateTime($arItem[$arParams["DATE_FIELD"].'_VALUE'], "YYYY-MM-DD", "ru")][] = $arItem;
    }
    ...
    if ($arParams["SHOW_YEAR"]) {
        $arFilter = array(
            "ACTIVE" => "Y",
            "<".$arParams["DATE_FIELD"] => date('Y-m-d', mktime(0, 0, 0, 1, 1, $currentYear)),
            "IBLOCK_ID" => $arParams["IBLOCK_ID"],
        );
    ...
    if (!$bPrevM) {
        $arFilter = array(
            "ACTIVE" => "Y",
            "<".$arParams["DATE_FIELD"] => date('Y-m-d', mktime(0, 0, 0, $currentMonth, 1, $currentYear)),
            "IBLOCK_ID" => $arParams["IBLOCK_ID"],
        );
        $dbItems = CIBlockElement::GetList(array(), $arFilter, false, array("nTopCount"=>1), array("ID", "IBLOCK_ID"));
        if ($arItem = $dbItems->GetNext()) {
    ...
    if ($bCheckNext) {
        if ($arParams["SHOW_YEAR"]) {
            $arFilter = array(
                "ACTIVE" => "Y",
                ">=".$arParams["DATE_FIELD"] => date('Y-m-d', mktime(0, 0, 0, 1, 1, $currentYear+1)),
                "IBLOCK_ID" => $arParams["IBLOCK_ID"],
            );
            $dbItems = CIBlockElement::GetList(array(), $arFilter, false, array("nTopCount"=>1), array("ID", "IBLOCK_ID"));
            if ($arItem = $dbItems->GetNext()) {
        ...
        if (!$bNextM) {
            $arFilter = array(
                "ACTIVE" => "Y",
                ">=".$arParams["DATE_FIELD"] => date('Y-m-d', mktime(0, 0, 0, $currentMonth+1, 1, $currentYear)),
                "IBLOCK_ID" => $arParams["IBLOCK_ID"]
            );
            $dbItems = CIBlockElement::GetList(array(), $arFilter, false, array("nTopCount"=>1), array("ID", "IBLOCK_ID"));
            if ($arItem = $dbItems->GetNext()) {
        ...
        for ($j = 0; $j < 7; $j++) {
            ...
            if (is_set($arDays[$tmpDate])) {
                ...
                foreach ($arDays[$tmpDate] as $dayNews) {
                    ...
                    if ($dayNews["PREVIEW_TEXT_TYPE"] == "text" && mb_strlen($dayNews["PREVIEW_TEXT"])>0) {
