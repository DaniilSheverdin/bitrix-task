...
JCCalendarInfoWin.prototype.deleteEntry = function()
{
	if (confirm(jsBXCalendarAdmin.DELETE_CONFIRM + ' "' + BX.util.htmlspecialcharsback(this.INFO.ENTRY.NAME) +'"?'))
	{
		...
		BX.ajax.post(
			'/local/components/citto/scud/tools/absence.php?action=delete'+ '&sessid=' + BX.message('bitrix_sessid') + '&absenceID='+ this.INFO.ENTRY.ID + '&js=1',
			BX.delegate(
				function()
				{
		...
		BX.AbsenceCalendar.popup = new BX.PopupWindow("BXAbsence", null, {
			...
			buttons: [
				new BX.PopupWindowButton({
					...
					events : { click : function()
					{
						...
						handler = BX.delegate(function(result) {
							...
							if (result[0] == '{')
							{
								let arJson= JSON.parse(result);

								let findNodes = document.querySelectorAll('#absence_'+arJson.id+' td');

								findNodes.forEach(function (v) {
									className = v.className;
									v.innerText = arJson[className];
								});
							...
							else if (/^error:/.test(result))
							{
								...
								let arJson = JSON.parse(document.querySelector('#jsonresult').innerText.trim());
								if (arJson.action == 'add') {
									let event = document.createElement('tr');
									event.id = 'absence_' + arJson.id;
									event.className = 'absence_event';
									event.addEventListener('click', function() {
										BX.AbsenceCalendar.ShowForm(
											{
												'ACTION': 'EDIT',
												'MESS': {
												...
												'HEAD_CONFIRM': arJson.headConfirm,
												'REASON': arJson.reason,
												'ABSENCE_ELEMENT_ID': arJson.id
											})
									});
									event.innerHTML = `
									<td class="user">${arJson.user}</td>
									<td class="headConfirm">${arJson.headConfirm}</td>
									<td class="dateRecord">${arJson.dateRecord}</td>
									<td class="dateAbsence">${arJson.dateAbsence}</td>
									<td class ="reason">${arJson.reason}</td>
								`;
									BX.AbsenceCalendar.popup.close();
									document.querySelector('#absences tbody').prepend(event);
								}
				...
				new BX.PopupWindowButtonLink({
					text: 'Удалить',
					className: "popup-window-button-delete ",
					events: { click : function()
					{
						BX.AbsenceCalendar.Delete()
					}}
				})
			],
			...
			events: {
				onAfterPopupShow: function()
				{
					...
					BX.ajax.post(
						'/local/components/citto/scud/tools/absence.php',
						{
...
BX.AbsenceCalendar.ShowForm = function(arParams)
{
	...
	button_delete = document.querySelector('.popup-window-button-delete');
	if (arParams.ACTION == 'EDIT') button_delete.style.display = '';
	else button_delete.style.display = 'none';

	BX.AbsenceCalendar.popup.show();
};

BX.AbsenceCalendar.Delete = function()
{
	if (confirm('Вы действительно хотите удалить запись?'))
	{
		BX.ajax.post(
			'/local/components/citto/scud/tools/absence.php',
			{
				sessid: BX.message('bitrix_sessid'),
				action:'delete',
				absenceID: BX.AbsenceCalendar.arParams.ABSENCE_ELEMENT_ID,
				js: 1
			},
			...
			BX.delegate(
				function(result)
				{
					...
					if (/^error:/.test(result))
					{
						...
						document.querySelector('#absence_'+BX.AbsenceCalendar.arParams.ABSENCE_ELEMENT_ID).remove();
						BX.AbsenceCalendar.popup.close();
					};
				},
				this
...
$(document).ready(function(){
	if (isAdmin && page == 'events') {
		var absenceID;

		$('body').on('click', '#absences tr', function(){
			absenceID = $(this).attr('id');
			if ($(this).find('.reason').text().length > 0)
				$('#delete_absence_modal').modal('show');
		});

		$('body').on('click', '#delete_absence_modal [data-event="yes"]', function(){
			let request = BX.ajax.runComponentAction('citto:scud', 'deleteAbsence', {
				mode: 'ajax',
				data: {
					action: 'deleteAbsence',
					sAbsenceID: absenceID
				}
			});
			request.then(function (data) {
				...
				if (data.data == 'error') {
					...
					$('#'+absenceID).find('.reason').text('');
					result = 'Нарушение удалено';
				}
				$('#result_absence_modal .result').text(result);
				$('#result_absence_modal').modal('show');
			});
		});
	}
});
