...
class CModuleOptions
{
    ...
    public function ShowHTML()
    {
        ...
        echo "<link rel=\"stylesheet\" type=\"text/css\" href=".'/local/modules/'.$module_id.'/assets/scripts/jquery-ui-1.12.1/jquery-ui.css'.">";
        echo "<script type=\"text/javascript\" src=".'/local/modules/'.$module_id.'/assets/scripts/jquery-3.4.1.min.js'."></script>";
        echo "<script type=\"text/javascript\" src=".'/local/modules/'.$module_id.'/assets/scripts/jquery-ui-1.12.1/jquery-ui.min.js'."></script>";
        echo "<script type=\"text/javascript\" src=".'/local/modules/'.$module_id.'/assets/scripts/script.js'."></script>";
        $arP = array();
            ...
            foreach ($this->arOptions as $option => $arOptParams) {
                ...
                switch ($arOptParams['TYPE']) {
                    ...
                    case 'SELECT':
                        ...
                        break;
                    case 'MSELECT':
                        $input = "<div class=\"ui-widget\">
                            <input name=".$opt." class='myInput'/><br>";
                        foreach ($this->arCurOptionValues[$option] as $item => $value) {
                            $input .= "<div><div data-user = $value style=\"border: 1px solid #9ea7b1;display: inline-block;border-radius: 3px;padding: 5px;\">".$arOptParams['VALUES']['REFERENCE'][array_search($value, $arOptParams['VALUES']['REFERENCE_ID'])]."<span class=\"close\" style=\"cursor: pointer;margin-left: 5px;font-weight: 700;color: red;\">x</span></div></div>";
                        }
                        $input .= "</div>";
                ...
                $input .= '<input id="p_ajax" type="hidden" value="'.'/local/modules/'.$module_id.'/ajax.php'.'">';
