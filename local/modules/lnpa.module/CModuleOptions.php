...
class CModuleOptions
{
    ...
    public function CModuleOptions($sModuleID, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
    ...
    private function SaveOptions()
    {
        foreach ($this->arOptions as $opt => $arOptParams) {
            if ($arOptParams['TYPE'] != 'CUSTOM') {
                ...
                COption::SetOptionString($this->sModuleID, $opt, $val);
            }
        }
    }
    ...
    private function GetCurOptionValues()
    {
        foreach ($this->arOptions as $opt => $arOptParams) {
            if ($arOptParams['TYPE'] != 'CUSTOM') {
                $this->arCurOptionValues[$opt] = COption::GetOptionString($this->sModuleID, $opt, $arOptParams['DEFAULT']);
        ...
        if (is_array($this->arOptions)) {
            foreach ($this->arOptions as $option => $arOptParams) {
                ...
                switch ($arOptParams['TYPE']) {
                    case 'CHECKBOX':
                        $input = '<input type="checkbox" name="' . $opt . '[]" id="' . $opt . '[]" value="Y"' . ($val == 'Y' ? ' checked' : '') . ' ' . ($arOptParams['REFRESH'] == 'Y' ? 'onclick="document.forms[\'' . $this->sModuleID . '\'].submit();"' : '') . ' />';
                        break;
                    case 'TEXT':
                        ...
                        $input = '<textarea id = "js-node-' . $opt . '" rows="' . $type[1] . '" cols="' . $arOptParams['COLS'] . '" rows="' . $arOptParams['ROWS'] . '" name="' . $opt . '">' . htmlspecialchars($val) . '</textarea>';
                        if ($arOptParams['REFRESH'] == 'Y') {
                            $input .= '<input type="submit" name="refresh" value="OK" />';
                        }
                        break;
                    case 'SELECT':
                        $input = SelectBoxFromArray($opt, $arOptParams['VALUES'], $val, '', '', ($arOptParams['REFRESH'] == 'Y' ? true : false), ($arOptParams['REFRESH'] == 'Y' ? $this->sModuleID : ''));
                        if ($arOptParams['REFRESH'] == 'Y') {
                            $input .= '<input type="submit" name="refresh" value="OK" />';
                        }
                        break;
                    case 'MSELECT':
                        $input = '<select multiple="multiple" class="selectpicker form-control" data-dropup-auto="false" data-size="5" title="Выбрать" data-live-search="true" data-live-search-placeholder="Найти" name="' . $option . '[]">';
                        foreach ($arOptParams["VALUES"] as $id => $value) {
                            $sSelected = "";
                            if (in_array($id, $val)) {
                                $sSelected = "selected";
                            }
                            $input .= "<option value='$id' $sSelected>$value</option>";
                        }
                        $input .= "</select>";
                        break;
                    case 'COLORPICKER':
                ...
                $input .= '<input id="p_ajax" type="hidden" value="' . '/local/modules/' . $this->sModuleID . '/ajax.php' . '">';
                ...
                $arP[$this->arGroups[$arOptParams['GROUP']]['TAB']][$arOptParams['GROUP']]['OPTIONS'][] = $label != '' ? '<tr id = "js-block-' . $opt . '"><td valign="top" width="40%">' . $label . '</td><td valign="top" nowrap>' . $input . '</td></tr>' : '<tr><td valign="top" colspan="2" align="center">' . $input . '</td></tr>';
            ...
            $tabControl->Begin();
            echo '<form name="' . $this->sModuleID . '" method="POST" action="' . $APPLICATION->GetCurPage() . '?mid=' . $this->sModuleID . '&lang=' . LANGUAGE_ID . '" enctype="multipart/form-data">' . bitrix_sessid_post();
