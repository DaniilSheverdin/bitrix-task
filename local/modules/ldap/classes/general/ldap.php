...
class CLDAP
{
	...
	function OnUserLogin(&$arArgs)
	{
		...
		elseif( $p > 0 )
		{
			$arFilter["CODE"] = mb_substr($LOGIN, 0, $p);
			$LOGIN = mb_substr($LOGIN, $p + 1);
		}
		...
		while($xLDAP = $db_ldap_serv->GetNextServer())
		{
			if($xLDAP->Connect())
			{
				...
				if(!$arLdapUser = $xLDAP->FindUser($LOGIN, $PASSWORD))
				{
					...
					if(isset($arArgs["OTP"]) && $arArgs["OTP"] <> '')
						if(mb_substr($PASSWORD, -6) == $arArgs["OTP"])
							$arLdapUser = $xLDAP->FindUser($LOGIN, mb_substr($PASSWORD, 0, -6));	//It can be with otp
				}
		...
		if($arLdapUser = $dbLdapUsers->Fetch())
		{
			if($PASSWORD !== false) // also check auth
			{
				...
				if(!empty($login_field)){
					if(isset($_REQUEST['AUTH_FORM']) && isset($_REQUEST['login']) && $_REQUEST['login'] == "Y"){
						if(!check_bitrix_sessid()) return false;
					}

					$ar_LOGIN_ATTEMPTS = $GLOBALS['DB']->Query('SELECT LOGIN_ATTEMPTS FROM b_user WHERE LOGIN = "'.$GLOBALS['DB']->ForSql($login_field).'"')->Fetch();
					if(!empty($ar_LOGIN_ATTEMPTS)){
						$usr_login_attempts = $ar_LOGIN_ATTEMPTS['LOGIN_ATTEMPTS'];
					}
					$usr_login_attempts++;
					$GLOBALS['DB']->Query('UPDATE b_user SET LOGIN_ATTEMPTS = '.$usr_login_attempts.', TIMESTAMP_X = TIMESTAMP_X WHERE LOGIN = "'.$GLOBALS['DB']->ForSql($login_field).'"');
					
					if($usr_login_attempts > 3){
						$_SESSION["BX_LOGIN_NEED_CAPTCHA"] = true;
						if(!$GLOBALS['APPLICATION']->CaptchaCheckCode($_REQUEST["captcha_word"], $_REQUEST["captcha_sid"])){
							$GLOBALS['APPLICATION']->ThrowException(GetMessage('CAPTCHA_WRONG'));
							return false;
						}
					}
				}
				...
				if (!$this->Bind($user_dn, $password_field)){
					if(!empty($login_field)){
						...
						$ar_LOGIN_ATTEMPTS = $GLOBALS['DB']->Query('SELECT LOGIN_ATTEMPTS FROM b_user WHERE LOGIN = "'.$GLOBALS['DB']->ForSql($login_field).'"')->Fetch();
						if(!empty($ar_LOGIN_ATTEMPTS)){
							$LOGIN_ATTEMPTS = $ar_LOGIN_ATTEMPTS['LOGIN_ATTEMPTS'];
						}
						$GLOBALS['DB']->Query('UPDATE b_user SET LOGIN_ATTEMPTS = '.($LOGIN_ATTEMPTS+1).', TIMESTAMP_X = TIMESTAMP_X WHERE LOGIN = "'.$GLOBALS['DB']->ForSql($login_field).'"');
					}
					return false;
				}
			}
	...
	public static function OnFindExternalUser($login)
	{
		...
		elseif( $p > 0 )
		{
			$filter["CODE"] = mb_substr($login, 0, $p);
			$login = mb_substr($login, $p + 1);
		}
