...
function _requestByubject(subject, providerData, __Callback) {
    ccadesPluginInit().then(function () {
        ...
        let providerName = providerNameCryptopro;
        let providerType = 0;
        ...
        cadesplugin.async_spawn(function* (args) {
            try {
                ...
                cryptoData.cryptoVersion = sVer;

                let CurrentPluginVersion = yield objVersion.PluginVersion;
                if (typeof(CurrentPluginVersion) == "undefined")
                    CurrentPluginVersion = yield objVersion.Version;

                let MajorVersion = yield CurrentPluginVersion.MajorVersion;
                let MinorVersion = yield CurrentPluginVersion.MinorVersion;
                let BuildVersion = yield CurrentPluginVersion.BuildVersion;
                cryptoData.pluginVersion = MajorVersion + "." + MinorVersion + "." + BuildVersion;

                let sCSPName = yield objVersion.CSPName(providerType);
                cryptoData.cryptoName = sCSPName;
                ...
                console.info(1, subject);
                console.info(2, strSubjectSignTool);
                console.info(3, cr);
                console.info(4, b64);
                ...
                $.post(
                    ...
                    {
                        message: 'Ошибка при формировании запроса на ключ ЭП',
                        params: {
                ...
                if (cryptoData.pluginVersion != '') {
                    /*
                     * https://cryptopro.ru/sites/default/files/products/cades/latest_2_0.txt
                     */
                    let currentVersion = '2.0.14071';

                    function VersionCompare_NPAPI(StringVersion, ObjectVersion)
                    {
                        var arr1 = StringVersion.split('.'),
                            arr2 = ObjectVersion.split('.');
                    ...
                    if (VersionCompare_NPAPI(currentVersion, cryptoData.pluginVersion) < 0) {
                        alert("КриптоПро ЭЦП Browser plug-in устарел.\nУстановлена версии " + cryptoData.pluginVersion + ".\nОбновите до версии не ниже " + currentVersion);
                    }
                }
                alert('Ошибка при формировании запроса на ключ ЭП (' + err.message + ')');
            }
        });
    });
...
function _handleByOptionselect(optionsList, id, props) {

    if(id == 'ep_oblast_primeneniya_klyucha_ep') {
        let setter = false;
        optionsList.forEach(
            function (i) {
                if (i == '1618') {
        ...
        if (setter) {
            props['ep_drugie_oblasti_v_kotorykh_nuzhna_podpis'].show = true;
        } else {
            props['ep_drugie_oblasti_v_kotorykh_nuzhna_podpis'].show = false;
        }
    }
...
var BPForm = function (_React$Component2) {
    ...
    _createClass(BPForm, [{
        ...
        value: function handleChange(_ref2) {
            ...
            _handleByOptionselect(fieldVal, fieldId, this.props.fields);
                    ...
                    if(_this5.state.sertRequest) {
                        ...
                        fetch(
                                    ...
                                    _requestByubject (sertStr, providerData, function(request) {
                                        ...
                                        fetch(
                                            ...
                                            .catch(function (error) {
                                                ...
                                                _this5.onErrror("Ошибка при формировании запроса на ключ ЭП (1)");
                                            });
                                    });
                                }
                            })
                            .catch(function (error) {
                                ...
                                _this5.onErrror("Ошибка при формировании запроса на ключ ЭП (2)");
                            });
                    } else {
                        _this5.form.current.submit();
                    }
