...
<script>
    $(document).ready(function () {
        ...
        function addDivForVacations() {
            $('#jq_massiv').detach();
            $('input[data-id="prichina"]').parent().after('<div id="jq_massiv" class="mb-3"><span>Дата и дни переноса отпуска</span>' + sNewString + '<a href="javascript: void(0);" id="add_vacation">Добавить ещё</a></div>');
        }
    ...
    bpFields.prichina.show = function () {
        return this.getValueXmlId('prichina_v_svyazi_s') != <?=json_encode(PRICHINA_SEM);?>
    };
