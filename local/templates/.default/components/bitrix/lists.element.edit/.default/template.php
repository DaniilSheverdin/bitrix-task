...
$this->addExternalJS($templateFolder . "/libs/polyfill/polyfill.min.js");
$this->addExternalJS($templateFolder . "/fetch.js");
            ...
            if ($arDocumentState["STATE_NAME"] <> '') {
                ...
                if ($arDocumentState["ID"] <> '') {
                    $arTab2Fields[] = array(
                        ...
                        "type" => "label",
                        "value" => '<a href="' . htmlspecialcharsbx($url) . '">' . ($arDocumentState["STATE_TITLE"] <> '' ?
                    ...
                    $arTab2Fields[] = array(
                        ...
                        "name" => GetMessage("CT_BLEE_BIZPROC_STATE"),
                        "type" => "label",
                        "value" => ($arDocumentState["STATE_TITLE"] <> '' ?
...
if (empty($arResult['ELEMENT']) && !empty($arResult['IBLOCK']['ID']) && file_exists(__DIR__ . '/forms/' . $arResult['IBLOCK']['ID'])) {
    if (
        false !== mb_strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') || 
        false !== mb_strpos($_SERVER['HTTP_USER_AGENT'], 'rv:11.0')
    ) {
        echo '<font color="red"><b>В браузере Internet Explorer могут возникать ошибки работы формы, рекомендуется использовать другой браузер</b></font><br/><br/>';
    }
    if (preg_match('/YaBrowser\/(.*) Yowser/usi', $_SERVER['HTTP_USER_AGENT'], $matches)) {
        if (mb_substr($matches[1], 0, 2) < 20) {
            echo '<font color="red"><b>Вы используете устаревшую версию Яндекс.Браузера (' . $matches[1] . '). Могут возникать ошибки работы формы, рекомендуется обновить браузер</b></font><br/><br/>';
        }
    }
    include __DIR__ . '/forms/' . $arResult['IBLOCK']['ID'] . "/template.php";
} else {
    $customDir = $templateFolder . '/custom/' . $arResult['IBLOCK']['ID'] . '/';
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $customDir . 'script.js')) {
        $this->addExternalJS($customDir . 'script.js');
    }
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . $customDir . 'style.css')) {
        $this->addExternalCss($customDir . 'style.css');
    }
