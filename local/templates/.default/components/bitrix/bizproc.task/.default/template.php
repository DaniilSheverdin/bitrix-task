...
if ($arResult['backUrl'] == SITE_DIR . 'company/personal/user/' . $GLOBALS['USER']->GetID() . '/') {
	$arResult['backUrl'] = SITE_DIR . 'company/personal/bizproc/';
}

$iElementID = (int) $arResult['TASK']['DOCUMENT_ID'];
$arElement = (CIBlockElement::GetByID($iElementID))->GetNext();
$sCodeIBLock = CIBlock::GetArrayByID($arElement['IBLOCK_ID'], "CODE");
$sRelativePathTaskJS = '/local/components/citto/bizproc/templates/' . $sCodeIBLock . '/task.js';
$sAbsolutePathCustomTaskJS = $_SERVER['DOCUMENT_ROOT'] . $sRelativePathTaskJS;

if (file_exists($sAbsolutePathCustomTaskJS)) {
    \Bitrix\Main\Page\Asset::getInstance()->addJs($sRelativePathTaskJS);
}

\Bitrix\Main\UI\Extension::load("ui.viewer");
\Bitrix\Main\Page\Asset::getInstance()->addCss('/bitrix/templates/.default/bootstrap.min.css');
\Bitrix\Main\Page\Asset::getInstance()->addJs('/local/js/bootstrap-plugin/popper.min.js');
\Bitrix\Main\Page\Asset::getInstance()->addJs('/bitrix/templates/.default/bootstrap.min.js');
...
<div class="bp-task-page bp-lent <?if (empty($arResult["TASK"]['STARTED_BY_PHOTO_SRC'])):?>no-photo<?endif?>">
		...
		elseif ($arResult["ShowMode"] == "Success"):
			switch ($arResult["TASK"]['USER_STATUS'])
			{
				case CBPTaskUserStatus::Yes:
					if(in_array(mb_strtolower($arResult['TASK']['ACTIVITY']), ["cbpcpproveactivity", "docsignactivity"])){
						echo '<span class="bp-status-ready"><span>Вы подписали документ</span></span>';
					}else{
	...
	<div class="bp-task-block">
		...
		if ($arResult["TASK"]["DESCRIPTION"] <> ''):
			...
			$arResult["TASK"]["DESCRIPTION"] = preg_replace('!\[color=([^\]]+?)\](.*?)\[\/color\]!i', '<span style="color:$1">$2</span>', $arResult["TASK"]["DESCRIPTION"]);
		...
		<p>
			...
			<a class="BPAT_GOTO_DOC" style="display:none" href="<?=$arResult["TASK"]["URL"]["VIEW"]?>" <?if ($arParams['POPUP']):?>target="_blank" <?endif?>><?=GetMessage("BPAT_GOTO_DOC")?></a>
			<?endif;?>
		</p>
		<?
	...
	<?endif?>
<?/*
				<script type="text/javascript" src="/bitrix/components/bitrix/main.post.list/templates/.default/script.js"></script>
				<script type="text/javascript" src="/bitrix/components/bitrix/main.post.list/templates/.default/scripts_for_form.js"></script>
				<script type="text/javascript" src="/bitrix/components/bitrix/main.post.list/templates/.default/scripts_for_form2.js"></script>
				<script type="text/javascript" src="/bitrix/components/bitrix/main.post.list/templates/.default/scripts_for_im.js"></script>
				<?
