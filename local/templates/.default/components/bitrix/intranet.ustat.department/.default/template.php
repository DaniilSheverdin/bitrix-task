...
if (IsModuleInstalled("bitrix24")) {
	$siteName = htmlspecialcharsbx(COption::GetOptionString("bitrix24", "site_title", ""));
} else {
	$siteName = htmlspecialcharsbx(COption::GetOptionString("main", "site_name", ""));
}

global $USER;
$arCurrentUser = Bitrix\Main\UserTable::getList([
    'select'    => ['ID', 'LID', 'UF_DEPARTMENT'],
    'filter'    => ['ID' => $USER->GetID()]
])->Fetch();

// МФЦ
if ($arCurrentUser['LID'] == Citto\Mfc::$siteId) {
    $siteName = Citto\Mfc::getName();
}
				...
				<table class="pulse-info-table">
					<tr>
						<td class="pulse-info-cell pulse-info-cell-top">
							...
							<div class="pulse-info-title"><?=$siteName;?></div>
						</td>
