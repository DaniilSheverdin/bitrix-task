...
/**
 * При установке галки Скрыть ДР стоит обработчик onBeforeUserUpdate
 * Он очищает PERSONAL_BIRTHDAY, нужно подсунуть сюда UF_PERSONAL_BIRTHDAY
 * При обновлении страницы это делает result_modifier
 */
BX.addCustomEvent("onEntityUpdate", BX.delegate(function(data){
	if (data.entityData.PERSONAL_BIRTHDAY == '') {
		if (data.entityData.UF_PERSONAL_BIRTHDAY.VALUE !== '') {
			data.entityData.PERSONAL_BIRTHDAY = data.entityData.UF_PERSONAL_BIRTHDAY.VALUE;
		}
	}
}, this));
