...
)
{
    ...
    <?
    /*
    if ($arResult["IsOwnProfile"] || $arResult["IS_CURRENT_USER_ADMIN"]) {
        $arPopupOptions = [
            'width' => 950,
        ...
        ?>
        <span
            onclick='BX.SidePanel.Instance.open("/company/personal/user/<?=$arResult["User"]["ID"];?>/my_bizproc/?create=1", <?=Json::encode($arPopupOptions)?>);'
            class="ui-btn ui-btn-light-border ui-btn-themes"
        >
...
<div class="intranet-user-profile" id="intranet-user-profile-wrap">
    <div class="intranet-user-profile-column-left">
        <div class="intranet-user-profile-column-block">
            ...
            <div
                class="intranet-user-profile-userpic ui-icon ui-icon-common-user
        ...
        */
        ...
        if ($arResult["IsOwnProfile"] || $arResult["IS_CURRENT_USER_ADMIN"]) {
            ...
            $personalUrl = SITE_DIR."company/personal/user/".$arResult["User"]["ID"]."/personal/";
            $careerUrl = SITE_DIR."company/personal/user/".$arResult["User"]["ID"]."/career/";
            $zayaviUrl = SITE_DIR."company/personal/user/".$arResult["User"]["ID"]."/zayavi/";
            $sUrlUserReport = SITE_DIR."company/personal/user/".$arResult["User"]["ID"]."/reports/";
            ...
            try {
                ...
                $arUserData = $obPersonal->getUserPortalData($arResult["User"]["ID"]);
                $arOnesData = [];
                if (!empty($arUserData['XML_ID'])) {
                    $arOnesData = $obPersonal->getUserOnesData($arUserData['XML_ID']);
                }
                if (!empty($arOnesData)) {
                    $hasPaySlip = !empty(trim($arUserData['UF_INN']));

                    $arPersonalButtons['PERSONAL'] = [
                        'PERSONAL',
                        'CAREER',
                    ];
                    if ($hasPaySlip) {
                        $arPersonalButtons['REFERENCE'][] = 'SETTLEMENT';
                    }
                    $arPersonalButtons['REFERENCE'][] = 'OBJECTIVE';
                }
            } catch (Exception $e) {
            ...
            <?if (!empty($arPersonalButtons['PERSONAL'])) : ?>
            <div class="intranet-user-profile-container">
                <div class="intranet-user-profile-container-header">
                    <div class="intranet-user-profile-container-title">Личная информация</div>
                </div>
                <div class="intranet-user-profile-container-body">
                    <div class="intranet-user-profile-container-body-text ui-btn-container ui-btn-container-center m-0">
                        <?if (in_array('PERSONAL', $arPersonalButtons['PERSONAL'])) : ?>
                        <button class="ui-btn ui-btn-success w-50" onclick='BX.SidePanel.Instance.open("<?=$personalUrl?>", <?=Json::encode($arPopupOptions)?>)'>Личные данные</button>
                        <?endif;?>
                        <?if (in_array('CAREER', $arPersonalButtons['PERSONAL'])) : ?>
                        <button class="ui-btn ui-btn-success w-50" onclick='BX.SidePanel.Instance.open("<?=$careerUrl?>", <?=Json::encode($arPopupOptions)?>)'>Моя карьера</button>
                        <?endif;?>
                    </div>
                </div>
            </div>
            ...
            <?if (!empty($arPersonalButtons['REFERENCE'])) : ?>
            <div class="intranet-user-profile-container">
                <div class="intranet-user-profile-container-header">
                    <div class="intranet-user-profile-container-title">Справки</div>
                </div>
                <div class="intranet-user-profile-container-body">
                    <div class="intranet-user-profile-container-body-text ui-btn-container ui-btn-container-center m-0">
                        <?if (in_array('OBJECTIVE', $arPersonalButtons['REFERENCE'])) : ?>
                            <button class="ui-btn ui-btn-success ui-btn-themes js-reference-lens w-50">Объективка</button>
                        <?endif;?>
                        <?if (in_array('SETTLEMENT', $arPersonalButtons['REFERENCE'])) : ?>
                            <button class="ui-btn ui-btn-success ui-btn-themes js-raschetnyy-listok w-50">Расчетный листок</button>
                        <?endif;?>
                    </div>
                </div>
            </div>
            ...
            <div class="intranet-user-profile-container intranet-user-profile-actions">
                <button class="ui-btn ui-btn-success w-50" onclick='BX.SidePanel.Instance.open("<?=$zayaviUrl?>", <?=Json::encode($arPopupOptions)?>)'>Заяви о себе</button>
            </div>
            <?endif;?>

            <div class="intranet-user-profile-container intranet-user-profile-actions">
                <button class="ui-btn ui-btn-success" onclick="window.open('/instructions/')">Должностная инструкция</button>
            </div>

            <div class="intranet-user-profile-container intranet-user-profile-actions">
                <button class="ui-btn ui-btn-success" onclick="window.open('/vaccination_covid19/')">Вакцинация COVID-19</button>
            </div>

            <?if (!empty($arPersonalButtons['BIZPROC'])) : ?>
            <div class="intranet-user-profile-container">
                <div class="intranet-user-profile-container-header">
                    <div class="intranet-user-profile-container-title">Бизнес-процессы</div>
                </div>
                <div class="intranet-user-profile-container-body">
                    <div class="intranet-user-profile-container-body-text ui-btn-container ui-btn-container-center m-0">
                        <?if (in_array('BIZPROC', $arPersonalButtons['BIZPROC'])) : ?>
                        <button class="ui-btn ui-btn-success w-50" onclick="window.open('/company/personal/bizproc/')">Задания БП</button>
                        <?endif;?>
                        <?if (in_array('PROCESSES', $arPersonalButtons['BIZPROC'])) : ?>
                        <button class="ui-btn ui-btn-success w-50" onclick="window.open('/company/personal/processes/')">Мои процессы</button>
                        <?endif;?>
                    </div>
                </div>
            </div>
        ...
        <? if ($arResult['REPORT_FILES']): ?>
            ...
            <? if (
                    $arResult["IsOwnProfile"] ||
                    $arResult["IS_CURRENT_USER_ADMIN"] ||
                    in_array($USER->GetID(), [5810, 1124])
            ): ?>
                <div class="intranet-user-profile-container intranet-user-profile-actions">
                    <button class="ui-btn ui-btn-success" onclick='BX.SidePanel.Instance.open("<?= $sUrlUserReport ?>", <?= Json::encode($arPopupOptions) ?>)'>Отчёты при
                        аттестации
                    </button>
                </div>
            <? endif; ?>
        ...
        <?
        $file = '/local/components/citto/profile.personal/templates/.default/raschetnyy-listok.php';
        include $_SERVER['DOCUMENT_ROOT'] . $file;
        $file = '/local/components/citto/profile.personal/templates/.default/reference-lens.php';
        include $_SERVER['DOCUMENT_ROOT'] . $file;

        global $USER;
        if ((!empty($arResult['USER_AUTH_SELECT']) && $arResult["User"]['ID'] == $USER->GetID()) || $USER->IsAdmin()) {
            ?>
        <div class="intranet-user-profile-container">
            <div class="intranet-user-profile-container-header">
                <div class="intranet-user-profile-container-title">Авторизация</div>
            </div>
            <div class="intranet-user-profile-container-body">
                ...
                <div class="intranet-user-profile-container-body-text">
                    <form method="post">
                        <div class="ui-ctl ui-ctl-after-icon ui-ctl-dropdown">
                            ...
                            <select class="ui-ctl-element" name="auth_user_id" id="auth_select">
                                <option value=<?=$USER->GetID()?>>
                                    <?=implode(' ', [$USER->GetLastName(), $USER->GetFirstName(), $USER->GetSecondName()])?>
                                </option>
                                <?php
                                foreach ($arResult['USER_AUTH_SELECT'] as $id => $fullName) {
                        ...
                        <br/>
                        <button class="ui-btn ui-btn-success" type="submit">Авторизоваться</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        ?>
        <?php if($USER->IsAdmin() || in_array($USER->GetID(), [1801, 593])) { ?>
            <div class="intranet-user-profile-container intranet-user-profile-actions">
                <button class="ui-btn ui-btn-success" onclick="javascript: window.location.href='/local/php_interface/bizproz/list_adaptaciy/index.php?uid=<? echo $arResult["User"]["ID"] ?>'">Отправить приветственное письмо</button>
            </div>
            <div class="intranet-user-profile-container intranet-user-profile-actions">
                <button class="ui-btn ui-btn-success" onclick="javascript: window.location.href='/local/php_interface/bizproz/mentoring/index.php?uid=<? echo $arResult["User"]["ID"] ?>'">Наставничество</button>
            </div>
        <?php } ?>
    </div>
    <div class="intranet-user-profile-column-right">
        ...
        $APPLICATION->IncludeComponent(
            ...
            array(
                ...
                "ENABLE_PERSONAL_CONFIGURATION_UPDATE" => false,//$arResult["EnablePersonalConfigurationUpdate"],
                "ENABLE_COMMON_CONFIGURATION_UPDATE" => false,//$arResult["EnableCommonConfigurationUpdate"],
                "ENABLE_SETTINGS_FOR_ALL" => false,//$arResult["EnableSettingsForAll"],
        ...
        if (
            !empty($arResult['User']['WORK_POSITION']) ||
            !empty($arResult['User']['UF_DEPARTMENT']) ||
            !empty($arResult["User"]["SUBORDINATE"]) ||
            !empty($arResult["User"]["MANAGERS"])
        ) {
        ?>
            <div class="intranet-user-profile-container">
                <div class="intranet-user-profile-container-header">
                ...
                if (!empty($arResult['User']['UF_DEPARTMENT'])) {
                    $arUserFields = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields("USER", $arResult["User"]["ID"], LANGUAGE_ID);
                    $arUserFields['UF_DEPARTMENT']["PROPERTY_VALUE_LINK"] = str_replace('#ID#', '#VALUE#', $arParams["PATH_TO_CONPANY_DEPARTMENT"]);
                    $arUserFields['UF_DEPARTMENT']['VALUE'] = array_merge(
                        $arUserFields['UF_DEPARTMENT']['VALUE'],
                        array_keys($arResult['User']['DEPARTMENTS'])
                    );
                    $arUserFields['UF_DEPARTMENT']['VALUE'] = array_unique($arUserFields['UF_DEPARTMENT']['VALUE']);
                    ?>
                <div class="intranet-user-profile-container-body">
                    <div class="intranet-user-profile-container-body-title">
                        Подразделени<?=count($arUserFields['UF_DEPARTMENT']['VALUE'])>1?'я':'е';?>
                    </div>
                    <div class="intranet-user-profile-container-body-text">
                        <?
