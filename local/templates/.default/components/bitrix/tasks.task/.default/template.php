						...
						<div class="task-options-item-open-inner">
							<? $APPLICATION->IncludeComponent(
								...
								[
									...
									'PATH_TO_USER_PROFILE' => $arParams['PATH_TO_USER_PROFILE'],
									'READ_ONLY' => ( \Bitrix\Tasks\Access\TaskAccessController::can(Util\User::getId(), \Bitrix\Tasks\Access\ActionDictionary::ACTION_TASK_CHANGE_RESPONSIBLE, $taskData['ID']) ? 'N' : 'Y'),
						...
						<div class="task-options-item-open-inner <?=$taskLimitExceeded? 'tasks-btn-restricted' : ''?>">
							...
							$APPLICATION->IncludeComponent(
								...
								array(
									...
									'READ_ONLY' => ( \Bitrix\Tasks\Access\TaskAccessController::can(Util\User::getId(), \Bitrix\Tasks\Access\ActionDictionary::ACTION_TASK_CHANGE_ACCOMPLICES, $taskData['ID']) ? 'N' : 'Y'),
								),
								false,
								array("HIDE_ICONS" => "Y", "ACTIVE_COMPONENT" => "Y")
							);
						...
						<?elseif($blockName == 'USER_FIELDS'):?>
							...
							<div class="task-options-item-open-inner">
								...
								$arExclude = array_merge(
									...
									ProjectInitiative::$arTaskUserFields
								);
								if ($taskData['ID'] > 0 && $taskData['GROUP_ID'] == ProjectInitiative::$groupId) {
									$arBizProc = ProjectInitiative::getBizProcByTaskId($taskData['ID']);
									if ($arBizProc['PROPERTY_TASK_ID_VALUE'] == $taskData['ID']) {
										$arExclude = array_diff($arExclude, ['UF_UTILISATION_SU', 'UF_JUSTIFICATION']);
									}
								}
								?>
								<?$APPLICATION->IncludeComponent(
