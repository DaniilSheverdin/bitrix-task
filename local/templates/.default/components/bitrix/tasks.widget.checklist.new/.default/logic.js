...
BX.Tasks.CheckList = (function()
{
	var CheckList = function(options)
	{
		...
		if (options.commonAction.canAddAccomplice) {
			var dateOptions = {
				year: 'numeric',
				month: 'numeric',
				day: 'numeric',
			...
			$('.tasks-checklist-item-flag-element').on('click', function(){
				...
				query.add(
					"task.update",
					{
						id: options.entityId,
						data: {
							CHANGED_DATE: new Date().toLocaleString("ru", dateOptions).replace(',', '')
						}
					}
				);
				query.execute();
