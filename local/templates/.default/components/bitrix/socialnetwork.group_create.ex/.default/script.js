...
BX.BXGCE.init = function(params) {
    ...
    if (BX('switch_additional'))
    {
        BX.bind(BX('switch_additional'), 'click', BX.delegate(function(e) {
            ...
            let className = 'social-group-create-field';
            $('.social-group-create-options-item select')
                .removeClass(className + ' ' + className + '-select')
                .addClass(className + ' ' + className + '-select');
            $('.social-group-create-options-item input')
                .removeClass(className + ' ' + className + '-input')
                .addClass(className + ' ' + className + '-input');

            let utilisation = $('input[name=UF_UTILISATION_SU]');
            if (utilisation.length > 0) {
                utilisation.on('keypress', function(event) {
                    event = event || window.event;
                    if (event.charCode && event.charCode != 0 && event.charCode != 46 && (event.charCode < 48 || event.charCode > 57) )
                        return false;
                });
                utilisation.on('input', function(event) {
