...
$arParams['GROUP_PROPERTY'][] = 'UF_SG_DEPT';
$arParams['~GROUP_PROPERTY'][] = 'UF_SG_DEPT';
...
while ($row = $res->Fetch()) {
    $arProjectUsers[] = $row['ID'];
}

if (in_array($arResult['POST']['OWNER_ID'], $arProjectUsers)) {
    $arParams['GROUP_PROPERTY'] = array_merge(
        $arParams['GROUP_PROPERTY'],
        \Citto\Tasks\ProjectInitiative::$arUserFields
    );
    $arParams['GROUP_PROPERTY'] = array_unique($arParams['GROUP_PROPERTY']);
    $arParams['~GROUP_PROPERTY'] = $arParams['GROUP_PROPERTY'];
}
...
if (
    ...
    foreach ($arResult["GROUP_PROPERTIES"] as $key => $userField) {
        if (!in_array($key, $arParams['GROUP_PROPERTY'])) {
            unset($arResult["GROUP_PROPERTIES"][ $key ]);
            continue;
        }
