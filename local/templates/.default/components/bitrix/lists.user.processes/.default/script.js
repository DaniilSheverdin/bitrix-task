...
function bp_stop(DOCUMENT_ID, WORKFLOW_ID){
	var document_rows = jQuery('.main-grid-row-body[data-id="'+DOCUMENT_ID+'"]');
	document_rows.css('opacity','0.3');
	
	jQuery.get(location.href,{ACTION:"bp_stop", DOCUMENT_ID:DOCUMENT_ID, WORKFLOW_ID:WORKFLOW_ID}, function(resp){
		if(resp.code == "OK"){
			document_rows.remove();
		}else{
			alert(resp.message);
			document_rows.css('opacity', 1);
		}
	},'json').fail(function(){
