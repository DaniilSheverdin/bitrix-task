...
if (isset($_REQUEST['controls'])) {
	...
	$control = $_REQUEST['controls']['action_button_' . $gridId ];
	$parentId = (int)$_REQUEST['controls']['parentId'];

	if ($control == 'setparent' && $parentId > 0) {
		$arTaskList = array_filter($_REQUEST['rows']);

		$arUpdate = [
			'PARENT_ID' => $parentId
		];
		foreach ($arTaskList as $tId) {
			if ($tId == $parentId) {
				continue;
			}
...
$arParams['COLUMNS'][] = 'PERCENT';
$arParams['COLUMNS'][] = 'START_DATE_PLAN';
$arParams['COLUMNS'][] = 'END_DATE_PLAN';

if ($_REQUEST['EXPORT_AS'] == 'EXCEL') {
    $arParams['COLUMNS'][] = 'DIFF_DATE';
    $arParams['COLUMNS'][] = 'TIME_FROM_VALUATION';
    $arParams['COLUMNS'][] = 'TIME_SPENT';
}
...
if (!function_exists('prepareHeaders'))
{
	...
	function prepareHeaders($arParams): array
	{
		...
		if ((int)$arParams['GROUP_ID'] == ProjectInitiative::$groupId) {
			$headers['PERCENT'] = array(
				'id' => 'PERCENT',
				'name' => Loc::getMessage('TASKS_COLUMN_PERCENT'),
				'sort' => false,
				'first_order' => 'desc',
				'editable' => false,
				'default'=> true
		...
		$headers['START_DATE_PLAN'] = array(
			'id' => 'START_DATE_PLAN',
			'name' => Loc::getMessage('TASKS_COLUMN_START_DATE_PLAN'),
			'sort' => false,
			'first_order' => 'desc',
			'editable' => false,
			'default'=> false
        ...
        $headers['END_DATE_PLAN'] = array(
            'id' => 'END_DATE_PLAN',
            'name' => Loc::getMessage('TASKS_COLUMN_END_DATE_PLAN'),
            'sort' => false,
            'first_order' => 'desc',
            'editable' => false,
            'default'=> false
        ...
        if ($_REQUEST['EXPORT_AS'] == 'EXCEL') {
            $headers['DIFF_DATE'] = array(
                'id' => 'DIFF_DATE',
                'name' => 'TASKS_EXCEL_DIFF_DATE',
                'sort' => false,
                'first_order' => 'desc',
                'editable' => false,
                'default'=> false
            ...
            $headers['TIME_FROM_VALUATION'] = array(
                'id' => 'TIME_FROM_VALUATION',
                'name' => 'TASKS_EXCEL_TIME_FROM_VALUATION',
                'sort' => false,
                'first_order' => 'desc',
                'editable' => false,
                'default'=> false
            ...
            $headers['TIME_SPENT'] = array(
                'id' => 'TIME_SPENT',
                'name' => 'TASKS_EXCEL_TIME_SPENT',
                'sort' => false,
                'first_order' => 'desc',
                'editable' => false,
                'default'=> false
...
if (!function_exists('formatPercentForOutput'))
{
	/**
	 * @param $row
	 * @throws Main\ObjectException
	 */
	function formatPercentForOutput(&$row): void
	{
		...
		if (isset($arTasksPercent[ $row['id'] ])) {
			$arPercent  = [
				0 => 0
			];
			...
			$row['columns']['PERCENT'] = number_format(array_sum($arPercent), 2, ',', '') . '%';
		}
	}
}
...
if (!function_exists('prepareGroupActionItems'))
{
	...
	function prepareGroupActionItems($arResult, $arParams): array
	{
		...
		$actionList[] = [
			'NAME' => Loc::getMessage('TASKS_LIST_GROUP_ACTION_SET_PARENT'),
			'VALUE' => 'setparent',
			'ONCHANGE' => [
				[
					'ACTION' => Grid\Panel\Actions::CREATE,
					'DATA' => [
						[
							'TYPE' => Grid\Panel\Types::CUSTOM,
							'ID' => 'action_set_parent_search',
							'NAME' => 'ACTION_SET_PARENT_SEARCH',
							'VALUE' => '<a id="task-supertask-link" class="webform-field-action-link">Выбрать</a>',
						],
						[
							'TYPE' => Grid\Panel\Types::HIDDEN,
							'ID' => 'action_set_parent_id',
							'NAME' => 'parentId',
						],
					],
				],
...
if (!empty($arResult['LIST']))
{
	...
	foreach ($arResult['LIST'] as $row)
	{
        ...
        if ($_REQUEST['EXPORT_AS'] == 'EXCEL') {
            $sDeadline = $row['DEADLINE'];
            $sEndDateplan = $row['END_DATE_PLAN'];

            if ($sDeadline && $sEndDateplan) {
                $obDeadline = date_create_from_format('d.m.Y H:i:s', $sDeadline);
                $obDateplan = date_create_from_format('d.m.Y H:i:s', $sEndDateplan);

                $obDateDiff = date_diff($obDateplan, $obDeadline);
                $row['DIFF_DATE'] = $obDateDiff->days;

                if ($obDateDiff->invert) {
                    $row['DIFF_DATE'] = '-' . $obDateDiff->days;
                }
            }

            $row['TIME_FROM_VALUATION'] = ($row['TIME_ESTIMATE']) ? $row['TIME_ESTIMATE'] / 3600 : '';
            $row['TIME_SPENT'] = ($row['TIME_SPENT_IN_LOGS']) ? $row['TIME_SPENT_IN_LOGS'] / 3600 : '';
        }

		$arResult['EXPORT_LIST'][ $row['ID'] ] = $row;
	}
	...
	foreach ($arResult['LIST'] as $key => $row)
	{
		...
		$data['columns']['UF_LAST_COMMENT'] = htmlspecialchars_decode($data['columns']['UF_LAST_COMMENT']);
		$data['columns']['UF_LAST_COMMENT'] = str_replace(
			[
				'[TABLE]', '[TR]', '[TD]',
				'[/TABLE]', '[/TR]', '[/TD]',
			],
			'',
			$data['columns']['UF_LAST_COMMENT']
		);

		if ($groupId == ProjectInitiative::$groupId) {
			formatPercentForOutput($data);
		}

		$arResult['EXPORT_LIST'][ $data['columns']['ID'] ]['PERCENT'] = $data['columns']['PERCENT'] ?? '';
		$arResult['EXPORT_LIST'][ $data['columns']['ID'] ]['START_DATE_PLAN'] = $row['START_DATE_PLAN'] ?? '';
		$data['columns']['START_DATE_PLAN'] = $row['START_DATE_PLAN'] ?? '';
		$arResult['EXPORT_LIST'][ $data['columns']['ID'] ]['END_DATE_PLAN'] = $row['END_DATE_PLAN'] ?? '';
		$data['columns']['END_DATE_PLAN'] = $row['END_DATE_PLAN'] ?? '';
