...
<div id="<?=$helper->getScopeId()?>" class="task-view-buttonset <?=implode(' ', $arResult['CLASSES'])?>">
	...
	<span data-bx-id="task-view-b-buttonset">
		...
		if ($taskData['GROUP_ID'] == \Citto\Tasks\ProjectInitiative::$groupId) {
			if (\Citto\Tasks\ProjectInitiative::canCreateGroup($taskData['ID'])) {
				?><span data-bx-id="task-view-b-button" data-action="CREATE_GROUP"  class="task-view-button complete js-create-group ui-btn ui-btn-success">
					Создать на основании проект
				</span><?
			}
		}
