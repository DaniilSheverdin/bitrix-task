...
$this->addExternalCss($templateFolder."/css/select2.css");
$this->addExternalJS($templateFolder.'/js/select2.min.js');
...
$arResult['CAN_EDIT_USER'] = ($arResult['CAN_EDIT_USER'] || $USER->GetID() == $arUser['ID']) && !in_array($arUser["EXTERNAL_AUTH_ID"], Array('imconnector', 'bot'));
// if(!defined('DEV_SERVER')){
	// if($_SERVER['SERVER_NAME'] != "corp.tularegion.local"){
...
<div class="user-profile-block-wrap">
	...
	?>
		<div class="user-profile-events">
			...
			<? else : ?>
					...
					<?endif;?>
                <a href="/planner" class="user-profile-events-item user-profile-codes"><i></i>Планирование отпусков</a>
			<? endif; ?>
			</div>
			...
			?></div>
			<div class="user-profile-events-title">Бизнес-процессы</div>
			<div class="user-profile-events-cont">
				<?
	...
	<div class="user-profile-block-wrap-cont">
		<table class="user-profile-block" cellspacing="0">
			<?php
			if($arResult['CAN_EDIT_USER']||$_REQUEST['debug']=='Y'){
	...
	<!-- // otp -->
				...
				<?
				/*if(!empty($arResult['PERSONAL_DATA_DateOfBirth']) && empty($arResult['User']['UF_DATEOFBIRTHHIDE'])): ?>
					<tr>
						<td class="user-profile-nowrap">Дата рождения:</td>
						<td>
							<div style="margin-bottom:5px;"><?=$arResult['PERSONAL_DATA_DateOfBirth']?></div>
						</td>
					</tr>
				<?endif;*/?>
	...
	<?
			...
			<? } ?>
		<?}?>
            <? if ($arResult['USER_AUTH_SELECT'] && $arUser['ID'] == $USER->GetID() || $USER->IsAdmin()): ?>
            <tr>
                <td class="user-profile-block-title" colspan="2"><?=GetMessage("SONET_AUTHORIZE_AS_OTHER_USER")?></td>
            </tr>
            <tr>
                <form action="" method="post">
                    <td>
                        <select name="user_id" id="auth_select">
                            <option value=<?=$USER->GetID()?>><?=implode(' ', [$USER->GetLastName(), $USER->GetFirstName(), $USER->GetSecondName()])?></option>
                    ...
                    <td>
                        <button class="button-auth-change" type="submit"><?=GetMessage("SONET_AUTHORIZE_AS_OTHER_USER_ACTION")?></button>
                    </td>
                </form>
            </tr>
            <?endif;?>
