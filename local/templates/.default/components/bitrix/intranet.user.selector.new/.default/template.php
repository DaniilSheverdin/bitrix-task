...
$ajaxPath = $this->__component->GetPath();
...
if ($arCurrentUser['LID'] == Citto\Mfc::$siteId) {
    $arParams["SITE_ID"] = Citto\Mfc::$siteId;
    $depId = Citto\Mfc::$structureSectionId;
    $ajaxPath = $this->__folder;
    $arResult['STRUCTURE'][0][0] = $depId;
    $arResult['SECTIONS'] = [
        $depId => [
            'ID'    => $depId,
            'NAME'  => Citto\Mfc::getName()
        ]
    ];
}

$ajaxUrl = $ajaxPath . '/ajax.php?' .
    http_build_query(
        array(
            'lang'                => LANGUAGE_ID,
