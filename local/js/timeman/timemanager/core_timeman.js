;(function() {
	...
	BX.CTimeManWindow.prototype.CreateMainRow = function(DATA)
	{
		...
		if (DATA.STATE != 'CLOSED' || DATA.CAN_OPEN)
		{
			...
			DATA.CAN_EDIT = false; // @todo: Запрещаем редактирование времени вручную
			...
			DATA.CAN_EDIT = true; // @todo: Запрещаем редактирование времени вручную
		}
		else
		{
	...
	*/
	...
	{
		//@todo: end of check report
		if(e.target.parentNode.className.indexOf('webform-small-button-decline') > -1 && this.REPORT.REPORT_TEXT.length == 0) {
			var popup = BX.PopupWindowManager.create("popup-message", null, {
				content: "Вы не написали отчет по работе",
				darkMode: true,
				autoHide: true
			});
			popup.show();
