...
class EanUpc extends \Mpdf\Barcode\AbstractBarcode implements \Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		...
		if ($upce) {
			...
			$prodCode = (int)mb_substr($code, 7, 5); // product code
			$invalidUpce = false;
			if (($tmp == '000') or ($tmp == '100') or ($tmp == '200')) {
				// Manufacturer code ends in 000, 100, or 200
				$upceCode = mb_substr($code, 2, 2) . mb_substr($code, 9, 3) . mb_substr($code, 4, 1);
				if ($prodCode > 999) {
					$invalidUpce = true;
				}
			} else {
				...
				if ($tmp == '00') {
					...
					$upceCode = mb_substr($code, 2, 3) . mb_substr($code, 10, 2) . '3';
					if ($prodCode > 99) {
						$invalidUpce = true;
					}
				} else {
					...
					if ($tmp == '0') {
						...
						$upceCode = mb_substr($code, 2, 4) . mb_substr($code, 11, 1) . '4';
						if ($prodCode > 9) {
							$invalidUpce = true;
						}
					} else {
						// Manufacturer code does not end in zero
						$upceCode = mb_substr($code, 2, 5) . mb_substr($code, 11, 1);
						if ($prodCode > 9) {
							$invalidUpce = true;
						}
					}
