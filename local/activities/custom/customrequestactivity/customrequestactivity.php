...
class CBPCustomRequestActivity
	extends CBPActivity
	implements IBPEventActivity, IBPActivityExternalEventListener
{
	const ACTIVITY = 'CustomRequestActivity';
	...
	public function __construct($name)
	{
		...
		$this->arProperties = array(
			...
			"Changes" => array(),
			"CheckParameter"=>null,
			"CheckParameterValue"=>null,
			"ArrNonParameters"=>array()
		);
	...
	public static function CorrectNPArray($str)
	{
		$arr_ = explode(",", $str);
		$arr = array();
		foreach ($arr_ as $v)
	...
	public function Subscribe(IBPActivityExternalEventListener $eventHandler)
	{
		...
		$arParameters["ARR_NON_PARAMETER"] = $this->ArrNonParameters;
		$arParameters["CHECK_PARAMETER"]= $this->CheckParameter;
		$arParameters["CHECK_PARAMETER_VALUE"] = $this->CheckParameterValue;
	...
	public static function ShowTaskForm($arTask, $userId, $userName = "", $arRequest = null)
	{
		...
		$arrNonParameters = $arTask["PARAMETERS"]["ARR_NON_PARAMETER"];
		$checkParameter = $arTask["PARAMETERS"]["CHECK_PARAMETER"];
		$checkParameterValue = $arTask["PARAMETERS"]["CHECK_PARAMETER_VALUE"];
			...
			&& $arTask["PARAMETERS"]["REQUEST"] && is_array($arTask["PARAMETERS"]["REQUEST"]) && count($arTask["PARAMETERS"]["REQUEST"]) > 0)
		{
			foreach ($arTask["PARAMETERS"]["REQUEST"] as $parameter)
			{
				...
				if ((!empty($checkParameter))&&(!empty($arrNonParameters)))
				{
					if ($parameter["Name"]==$checkParameter)
					{
						foreach ($arrNonParameters as $NonParameter)
						{

							$form .= '<script type="text/javascript">BX.bind(BX(\'id_'.$parameter['Name'].'\'), \'change\',
								function()
								{

								if (this.value==\''.$checkParameterValue.'\')
									{
										BX.hide(BX(\'id_'.$NonParameter.'\').closest(\'tr\'));
									}
								else
									{
										BX.show(BX(\'id_'.$NonParameter.'\').closest(\'tr\'));
									}
								}
								)
								</script>';
	...
	protected static function getTaskResponse($task)
	{
			...
			&& $task["PARAMETERS"]["REQUEST"] && is_array($task["PARAMETERS"]["REQUEST"]) && count($task["PARAMETERS"]["REQUEST"]) > 0)
		{
			...
			$arrNonParameters = $task["PARAMETERS"]["ARR_NON_PARAMETER"];
			$checkParameter = $task["PARAMETERS"]["CHECK_PARAMETER"];
			$checkParameterValue = $task["PARAMETERS"]["CHECK_PARAMETER_VALUE"];

			if ((!empty($checkParameter))&&(!empty($checkParameterValue))&&(!empty($arrNonParameters)))
			{
				if ($request[$checkParameter] == $checkParameterValue)
				{
					foreach ($arrNonParameters as $iL=>$nonPar)
					{
						$request[$nonPar] = '0'; //$arrNonParametersValue[$iL];
					}
				}
			}
	...
	public static function GetPropertiesDialog($documentType, $activityName, $arWorkflowTemplate, $arWorkflowParameters, $arWorkflowVariables, $arCurrentValues = null, $formName = "", $popupWindow = null)
	{
		...
		$arMap = array(
			...
			"CheckParameter"=>"check_parameter",
			"CheckParameterValue"=>"check_parameter_value",
			"ArrNonParameters"=>"arr_non_parameters"
		);
			...
			if (is_array($arCurrentActivity["Properties"]))
			{
				foreach ($arMap as $k => $v)
				{
					if (array_key_exists($k, $arCurrentActivity["Properties"]))
					{
						...
						elseif($k == "ArrNonParameters")
							$arCurrentValues[$arMap[$k]] = implode(",",$arCurrentActivity["Properties"][$k]);
						else
	...
	public static function GetPropertiesDialogValues($documentType, $activityName, &$arWorkflowTemplate, &$arWorkflowParameters, &$arWorkflowVariables, $arCurrentValues, &$arErrors)
	{
		...
		$arMap = array(
			...
			"check_parameter"=>"CheckParameter",
			"check_parameter_value"=>"CheckParameterValue",
			"arr_non_parameters"=>"ArrNonParameters"
		...
		foreach ($arMap as $key => $value)
		{
			if ($key == "requested_users")
				continue;
			if ($key == "arr_non_parameters")
				$arProperties[$value] = self::CorrectNPArray($arCurrentValues[$key]);
			else
