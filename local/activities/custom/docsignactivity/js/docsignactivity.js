...
var docsignInit = function(){
    setTimeout(function(){
		...
		function signInput(inputs,certinfo){
			if(inputs.length == 0){
			...
			if(input.className == "signed"){
				signInput(inputs,certinfo);
				return;
			}

			input.className = "signing";
			SignCreate(certinfo.oCertificate, input.value, input.getAttribute('data-type') == "p7s", input.getAttribute('data-source')).then(function(data){
				$.post("/local/activities/custom/docsignactivity/ajax.php",{
					id:input.getAttribute('data-id'),
					file:data,
					certinfo:Object.assign({}, certinfo, {oCertificate:""})
				},function(resp){
					if(resp.code != "OK"){
		...
		function showCerts(){
			window.FillCertList_NPAPI().then(
				function(certList) {
					...
					docsogn_certlist.find('option:eq(0)').prop('selected',true);
				},
				function(error) {
					console.log(error);
		...
		function init(){
			window.cadesplugin.then(function(){
				if($('.docsign-files input:not(.signed)').length == 0){
					$('.docsign-actions, .docsign-cryptoplugin').hide();
					connectionStatus("Файлы подписаны", "success");
				}else{
					initialized = true;
					showCerts(); 
					...
					$('#docsign__sign-files').click(function(){
						...
						signInput(
							inputs,
							$('.docsign-cryptoplugin__certs select option:selected').data('certinfo')
						);
					});
				}
			}, function(err){
