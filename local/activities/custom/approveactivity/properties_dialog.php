...
<tr>
    <td align="right" width="40%">
        Отправлять в АСЭД Дело:
    </td>
    <td width="60%">
        <script>
        function __BPToggleFilesForm(v)
        {
            if (v == 'Y') {
                try {
                    document.getElementById("approve_files").style.display = 'table-row';
                } catch(e) {
                    document.getElementById("approve_files").style.display = 'block';
                }
            } else {
                document.getElementById("approve_files").style.display = 'none';
            }
        }
        </script>
        <select name="send_to_ased" onchange="__BPToggleFilesForm(this.value)">
            <option value="N"<?= $arCurrentValues["send_to_ased"] != "Y" ? " selected" : "" ?>><?= GetMessage("BPAA_PD_NO") ?></option>
            <option value="Y"<?= $arCurrentValues["send_to_ased"] == "Y" ? " selected" : "" ?>><?= GetMessage("BPAA_PD_YES") ?></option>
        </select>
    </td>
</tr>
<tr style="<?=$arCurrentValues["send_to_ased"] != "Y"?'display:none':''?>" id="approve_files">
    <td align="right" width="40%">
        <span class="adm-required-field">Файлы:</span><br/>
        <small style="color:red">Файлы не подписываются!</small>
    </td>
    <td width="60%">
