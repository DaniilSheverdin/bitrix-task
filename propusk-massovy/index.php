...
define('BP_IBLOCK', 491);
define('BP_TEMPLATE_ID', 381);
...
$APPLICATION->AddHeadScript('/propusk-massovy/index.js');
$APPLICATION->SetAdditionalCSS('/propusk-massovy/main.css');
...
if (isset($_REQUEST['propusk-massovy'])) {
    ...
    try {
        ...
        $sTableMCList = '';
        if (!empty($_REQUEST['MC_NAIMENOVANIE_MC'])) {
            $sTableMCList = '<table border="1" cellpadding="3" cellspacing="0" style="border-color: #999; margin: 20px 0;">';
            $sTableMCList .= '<tr><td border="1" style="border-color: #999;">Название МЦ</td><td border="1" style="border-color: #999;">Инвентарный номер МЦ</td></tr>';
            $sNaimenovaniemc = '';
            foreach ($_REQUEST['MC_NAIMENOVANIE_MC'] as $ind => $item) {
                $sTableMCList .= '<tr><td border="1" style="border-color: #999;">' . $item . '</td><td border="1" style="border-color: #999;">' . $_REQUEST['MC_INVENTARNY_NOMER'][$ind] . '</td></tr>';
                $sNaimenovaniemc .= $item . ' (инв № ' . $_REQUEST['MC_INVENTARNY_NOMER'][$ind] . '), ' . PHP_EOL;
            }
            $sTableMCList .= '</table>';
        }
        ...
        $arProps = [
            ...
            'MC_NAIMENOVANIE_MC' => $sNaimenovaniemc,
        ];
        ...
        $zayavka_file_props['MC_NAIMENOVANIE_MC'] = $sTableMCList;
...
<form class="needs-validation propusk-massovy" novalidate="" style="" id="propusk-massovy" action="<?=htmlspecialchars($_SERVER['REQUEST_URI'])?>" method="POST" autocomplete="Off">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control" name="VYBOR_ZDANIYA" required onchange="">
    ...
    <?if (BP_PROPUSK_VODY):?>
        ...
        <div class="form-group row">
            <label class="col-sm-2 col-form-label" for="bp_MC_NAIMENOVANIE_MC">Список МЦ</label>
            <div class="col-sm-10">
                <div class="col-12 mb-3" data-mc="1">
                    <div class="row pb-3 alert alert-primary my-2">
                        <div class="col-xl-6">
                            <input class="form-control" placeholder="Название МЦ" name="MC_NAIMENOVANIE_MC[]" value="" id="bp_MC_NAIMENOVANIE_MC" />
                        </div>
                        <div class="col-xl-6">
                            <input class="form-control" placeholder="Инвентарный номер МЦ" name="MC_INVENTARNY_NOMER[]" value="" />
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 py-2"><button id="add-mc" type="button" title="Добавить МЦ" class="btn btn-primary float-right">+</button></div>
        </div>
        <div class="form-group row">
    ...
    <?else:?>
        <div id="js-phone" class="form-group row">
            <label class="col-sm-2 col-form-label ">Контактный телефон</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name="KONTAKTNYY_TELEFON">
            </div>
