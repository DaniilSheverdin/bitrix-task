...
if(getenv('HTTP_HOST') == 'corp.tularegion.local') {
    define('TMP', $_SERVER['DOCUMENT_ROOT'] . "/../newcorp_arch/mpdf_arch");
} else {
    define('TMP', dirname($_SERVER['DOCUMENT_ROOT']) . "/newcorp_arch/mpdf_arch");
}
 ...
 * @throws  Exception
 */
$result_pdf_content = function (string $html, ?string $src, ?array $mpdf_params) use ($tempnam, &$cropImage) {
    try {
