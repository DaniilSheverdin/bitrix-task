...
$bOnlyBizProc = CSite::InGroup([135]);
...
if ($bLandingIncluded)
{
    ...
    if ($GLOBALS['USER']->IsAdmin() && Rights::hasAdditionalRight(Rights::ADDITIONAL_RIGHTS["menu24"], "knowledge"))
    {
        $arMenuB24[] = array(
...
if (!$bOnlyBizProc && CModule::IncludeModule("socialnetwork"))
{
    $canCreateGroup =
    ...
    $arMenuB24[] = array(
        ...
        "CBXFeatures::IsFeatureEnabled('Workgroups')"//" && (CSite::InGroup([26, 85, 87, 88]) || SITE_ID == 'nh')"
    );
}
if (
    \Bitrix\Main\Loader::includeModule('rpa')
    && \Bitrix\Rpa\Driver::getInstance()->isEnabled()
    && (CSite::InGroup([1,121]) || in_array($userId, [3462, 2547]))
) {
    $arMenuB24[] = [
...
foreach ($aMenuLinks as $arItem) {
    ...
    if (preg_match("~/(workgroups|crm|marketplace|docs|timeman|company|about|services)/$~i", $menuLink)) {// bizproc|
        continue;
    }
...
if (!$bOnlyBizProc && file_exists($_SERVER["DOCUMENT_ROOT"].SITE_DIR . "about/"))
{
    $arMenuB24[] = array(
...
//     ),
//     "IsModuleInstalled('rest') && SITE_ID == 'nh'"
// );
...
$arMenuB24[] = array(
    ...
    array(
        "real_link" => getLeftMenuItemLink(
            "top_menu_id_business_news",
            SITE_DIR . "about/business_news.php"
        ),
        "menu_item_id" => "menu_business_news_sect",
        "top_menu_id" => "top_menu_id_business_news"
    ),
    "SITE_ID == 'nh'"
);
...
if (!$bOnlyBizProc) {
    $arMenuB24[] = array(
        "Контроль поручений",
        "/control-orders/?action_filter=1138",
        array(),
        array(
            "real_link" => getLeftMenuItemLink(
                "top_menu_id_control_orders",
                "/control-orders/?action_filter=1138"
            ),
            "menu_item_id" => "menu_control_orders_sect",
            "top_menu_id" => "top_menu_id_control_orders"
        ),
        'CSite::InGroup([98])'
    );
    ...
    $arMenuB24[] = array(
        ...
        'CSite::InGroup([96, 97, 108, 109, 128])'
    );
    ...
    $arMenuB24[] = array(
        ...
        array(
            "counter_id" => "planner",
            "menu_item_id" => "menu_planner",
            "my_tools_section" => true,
        ),
        ''//(bool)array_intersect(CIntranetUtils::GetUserDepartments($GLOBALS["USER"]->GetId()), [453, 1722, 1721])'
    );
    /*
    $arMenuB24[] = array(
        ...
        array(),
        array(
            "counter_id" => "services_mer",
            "menu_item_id" => "menu_services_mer",
            "my_tools_section" => true,
        ),
        '(bool)array_intersect(array($GLOBALS["USER"]->GetId()), [5,35,570,418,576,74,546]) && (SITE_ID == "nh")'
    );
    */
    /*
    $arMenuB24[] = array(
        ...
        array(),
        array(
            "counter_id" => "services_in",
            "menu_item_id" => "menu_services_in",
            "my_tools_section" => true,
        ),
        'SITE_ID == "nh"'
    );
    */
    $arMenuB24[] = array(
        ...
        '(bool)in_array($GLOBALS["USER"]->GetId(), [5, 10])'
    );
    ...
    $arMenuB24[] = array(
        ...
        array(
            "counter_id" => "gallery",
            "menu_item_id" => "menu_gallery",
            "my_tools_section" => true,
        ),
        "(bool)array_intersect(array(SITE_ID), ['s1'])"
    );
    ...
    $arMenuB24[] = array(
        "Удаленная работа",
        SITE_DIR . "/docs/manage/path/Удаленная%20работа/",
        array(),
        array(
            "counter_id" => "home_work",
            "menu_item_id" => "menu_home_work",
            "my_tools_section" => true,
        ),
        ""
    );
    ...
    $arMenuB24[] = array(
        ...
        array(
            "counter_id" => "kpi",
            "menu_item_id" => "menu_kpi",
            "my_tools_section" => true,
        ),
        '(bool)in_array($GLOBALS["USER"]->GetId(), [1460,211,107,56,60,611,74,65,76,580,204,566,368])'
    );
    $arMenuB24[] = array(
        "Запись на вакцинацию",
        SITE_DIR . "/vaccination",
        array(),
        array(
            "counter_id" => "vaccination",
            "menu_item_id" => "menu_vaccination",
            "my_tools_section" => true,
        ),
        ''
    );
